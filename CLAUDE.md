...
src/lib/services/zoho-service.ts    ← THE two Zoho reads: getBooksOverview() (/books, ~13 calls, Redis 5 min) + getClientFinance(zohoCustomerId) (the client finance page, 4 calls, Redis 1 min) + their invalidations; nothing from Zoho lands in Postgres. Contract: docs/integrations/zoho-books.md
...
src/components/books/               ← THE Books surface (admin/founder): BooksOverview (the tile strip + accounts), ZohoTables (invoices / payments / credit notes over Table<T>, rows link to Zoho), ZohoStatusPill, RefreshBooksButton; the client finance page composes the same tables (ClientZohoCards)
src/lib/services/client-observation-reader.ts ← readClientObservation() — THE free-form → facts reader (routing tier via the Elaya provider + maskPii; fails closed for facts, open for the note). addObservationCore in client-mutations.ts is the ONLY caller; ClientObservationCard is THE way facts enter a client (the facts cards only "correct")
src/lib/constants/client-facets.ts  ← THE client-twin vocabulary (0194): CLIENT_FACETS / FACT_SOURCES / polarity / CLIENT_EVENT_KINDS / RELATION_* / ANTICIPATION_KINDS / CLIENT_TIERS + tierFromLabel + computeHealthScore (baseline 70, per-signal half-life decay). The SQL CHECKs mirror these lists — a new value = one entry + a CHECK migration
...
scripts/clients/seed-client-facts.py ← THE STORE-2 seeder (Atlas + Typeform CSVs, Freshdesk contact preferences, the sheets' city/company → client_facts with source/confidence/evidence; idempotent, dry-run default). scripts/import-clients-and-map-groups.py stays THE spine importer (now reads export-1/export-2.csv too)
src/lib/services/clients-service.ts ← ALL /clients reads (session client; RLS client_visible = the whole queendom + admin/founder): listClients, getClientDetail, searchClientsForPicker, getQueendoms. The mirror's tickets and the Sia group come from their own homes (getFreshdeskTicketsForClient, getSiaGroupForClient)
src/lib/services/client-mutations.ts ← THE context-free client write cores (create/update spine, addFact with supersede, people, linkGroup, health adjust, logClientAccess); actions/clients.ts AND future Elaya tools call the SAME core; the caller checks canAccessClient (lib/elaya/access.ts) first
src/lib/actions/clients.ts          ← ALL client server actions (Zod → requireProfile → canAccessClient → core → revalidatePath); searchClientsAction = the picker
src/components/clients/             ← THE Clients surface: ClientsFilters / ClientsTable / ClientsTableSkeleton / AddClientButton + ClientFormModal (THE New/Edit client form) / HealthPill (THE score pill) / the dossier cards (Identity, Health, Facts ×2, Requests, WhatsApp, People, Notes, Activity, the sidebar cards) / ClientFinanceView (the /clients/[id]/finance body; gate = canSeeClientFinance in lib/elaya/access.ts, the one place to narrow money to a role)
...
src/lib/services/ticket-creator.ts  ← THE ticket creator (selected messages + the client twin → a TicketDraft; reasoning tier via the Elaya provider + maskPii; every call = sia.extraction_runs row; fails closed to null)
