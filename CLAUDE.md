...
src/lib/services/revival-gate.ts    ← (Lead Revival R1) judgeLeadForRevival() — THE note-AI suppression gate: ONE structured call reusing the Elaya provider/PII layer (resolveLlmForJob('routing') + maskPii, NO tools, NO new SDK import). Suppression bias in the prompt; fails CLOSED to 'unsure' — only an exact "revive" can become a task
