...
src/lib/services/elaya-jobs-service.ts ← THE elaya_jobs + elaya_labels access (0235): createElayaJob / getElayaJob / advanceElayaJob (queued → running → done|failed, service role only) / upsertElayaLabels (one label per subject per label_set; a re-read replaces) / countElayaLabels. Admin client; runs from Trigger.dev
src/lib/services/elaya-deep-read.ts ← runDeepRead(jobId) — THE deep read (0235): the question a chat turn cannot answer because the answer is in no column. plan (reasoning tier: which rows, which labels, the rubric; the fetch SQL is probed before anything is paid for) → fetch (the read-only door, keyset-paged, DEEP_READ_MAX_ROWS) → judge (routing tier, 100 rows a call, DEEP_READ_PARALLEL_CALLS side by side, maskPii) → labels written back to elaya_labels (the taxonomy grows by being asked) → aggregate in code → answer (reasoning tier) → delivered on the channel asked (the conversation as an assistant message, WhatsApp in parts, in-app). One sia.extraction_runs row per job with tokens + cost estimate. Queued by the write tool start_deep_read (founder/admin), run by src/trigger/elaya-deep-read.ts (startDeepReadJob). Never a keyword count presented as a judgement
