...
src/lib/services/vendor-mutations.ts ← THE context-free vendor write cores (create/update/status, mergeVendors (0223 — the merge_vendors RPC; seven tables and three UNIQUEs, so it is ONE transaction, never a sequence of calls from here), setVendorDeleted (0223 — deleted_at, NOT a DELETE: the ledger is ON DELETE RESTRICT and those rows are the record of money that moved), capability upsert/delete, logEngagement, closeEngagement — the ONE sanctioned write on the append-only ledger, resolve-once —, addReview, agent preference set/remove); take a MutationActor (reused from lead-mutations.ts); actions/vendors.ts AND future Elaya write tools call the SAME core — never re-implement a vendor write elsewhere
...
src/components/vendors/VendorAdminActions.tsx ← <VendorAdminActions vendor> — THE two non-additive vendor writes in one place (0223, admin/founder): Merge in (search → pick → fold another row into this one, through merge_vendors) and Remove / Restore (deleted_at, with a ConfirmDialog that says plainly that nothing is deleted). Mounted in the /vendors/[id] header; the page shows a warning banner when deleted_at is set, because a removed vendor still opens by direct link so it can be restored
