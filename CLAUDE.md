...
src/lib/actions/vendors.ts          ← ALL vendor server actions (Zod → requireProfile(['admin','founder']) → actorFromProfile → core → revalidatePath(VENDORS_PATH) → {data,error}); reads searchVendorsAction / getVendorDetailAction / rankVendorsAction / signVendorInvoiceAction
