...
src/lib/services/pulse-service.ts   ← getLivePulse() — THE live pulse: one company-wide picture (sales today, work, members waiting for a reply, Freshdesk), built from a read-only query over the elaya_read views + getFreshdeskOverview() + sia.groups_waiting_for_reply() (0224). Company-wide, so the CALLER gates (Elaya's get_live_pulse: founder/admin). No `server-only` chain (the briefing runs it from Trigger.dev)
src/lib/services/elaya-briefing.ts  ← runBriefingSweep(slot) — THE daily briefing: the pulse put into a few lines (routing tier via the Elaya provider + maskPii, fails OPEN to plainBriefing(), which is numbers only) → WhatsApp when the founder's 24-hour session window is open (a Meta template is needed outside it and does not exist yet) + always an in-app notification. src/trigger/elaya-briefing.ts = 09:00 and 19:00 IST, gated by elaya_settings.daily_briefing_enabled (0225, seeded false)
