...
src/lib/constants/vendors.ts        ← THE Vendors vocabulary (0183–0186, 0191): statuses / sources (ONE list for vendors.sources + engagements.source) / capability stances / PREFERENCE_STANCES (preferred/avoid — the sticky note) + PREFERRED_BOOST / outcomes / VENDOR_SERVICES (no SQL CHECK — grown as we learn) via defineEnum + REVIEW_DIMENSIONS (the 4 MANUAL ratings) + SCORE_WEIGHTS + window/limits + VENDOR_INVOICE_BUCKET + VENDORS_PATH
