...
src/components/tickets/             ← THE Tickets surface: TicketsFilters/Table/Skeleton, NewTicketForm (THE new-ticket page body; TICKET_SELECTION_KEY = the sessionStorage hand-off from the Sia chat), TicketHeaderControls (status/priority/assignee), TicketBriefCard, TicketChecklistCard, TicketTimeline (events + notes), TicketMoneyCard, TicketSideCards (linked words, sub-work, THE help window), TicketStatusPill
