...
src/lib/services/elaya-jobs-service.ts ← THE elaya_jobs + elaya_labels access (0235): createElayaJob / getElayaJob / advanceElayaJob (ONE call per status: it stamps started_at / finished_at) / updateElayaJobProgress (the mid-run pings, stamps nothing) / upsertElayaLabels (one label per subject per label_set; a re-read replaces) / getExistingLabels (a set's saved verdicts, keyset-paged through the door past the 5,000 cap) / listLabelSets (every set with the plan that made it) / getActiveLabelSets / countElayaLabels (counted in SQL — a plain select stops at PostgREST's 1,000-row response cap and calls that the total). Admin client; runs from Trigger.dev
src/lib/services/elaya-deep-read.ts ← runDeepRead(jobId) — THE deep read (0235; made fast and fail-safe 2026-09-24): the question a chat turn cannot answer because the answer is in no column. plan (reasoning tier; the label sets on record are shown so the SAME question reuses the same name, labels, rubric and query — a different label list under a known name is renamed _v2; windows relative to now(); a refresh_sql for the nightly top-up; the SQL is probed before anything is paid for) → COUNT (a BOUNDED count that stops one row past DEEP_READ_MAX_ROWS, then count(distinct subject_id): above the cap, or a count that times out, the job is REFUSED with a line asking for a narrower scope, never cut at the cap) → fetch (the read-only door, keyset-paged, de-duplicated on subject_id; the rows read must cover DEEP_READ_COVERAGE_MIN_PCT of the count or the job FAILS and says so) → REUSE (a row whose text still matches its saved evidence keeps its verdict; only new or changed rows are judged) → judge (routing tier, 100 rows a call answered by LETTER, a skipped row asked again on its own, DEEP_READ_PARALLEL_CALLS side by side behind the rate gate: a 429/overloaded pauses EVERY worker for retry-after or a doubling pause, never fails the batch on its own; maskPii) → labels written back → aggregate in code → answer (reasoning tier; names the reused rows, and the coverage / unjudged share only when below 100% / above 1%) → delivered on the channel asked. Above DEEP_READ_MAX_UNJUDGED_PCT unjudged = FAIL, no number. Real per-stage timings in progress + the ledger. planLabelRefresh() = the nightly top-up (src/trigger/elaya-labels-refresh.ts, 05:30 IST, ON unless elaya_labels_refresh_enabled is false): one `refresh` job per set asked about in LABELS_REFRESH_DAYS, judging only the rows it has not seen, delivering nothing. Queued by the write tool start_deep_read (founder/admin), run by src/trigger/elaya-deep-read.ts (startDeepReadJob). Never a keyword count presented as a judgement; never a number from a read that did not cover the rows
