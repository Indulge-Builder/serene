...
src/lib/services/member-profiler.ts ← openVault() = THE name-hiding vault (code names per group, masker, leak check, unmask) shared with ticket-draft-core.ts and ticket-intake.ts, never re-implemented · THE chat profiler (0215/0216): reads a LINKED member group's finished conversations (6 quiet hours ends one) and files facts / people / relations / coming-up into the member twin with message evidence. Names never reach the model: per-group code names (sia.codenames) + maskPii + a leak check that stops the reading. Reasoning tier via the Elaya provider (effort low); every call = a sia.extraction_runs row; fails closed (a failed reading does not move the group's bookmark). runProfilerSweep() is the one entry: src/trigger/member-profiler.ts (every 10 min, gated by elaya_settings.member_profiler_enabled; ON since 2026-09-18, Active members only via PROFILER_MEMBER_STATUSES → sia.profiler_due_groups 0218; a conversation that fails 3 readings is stepped over, a provider outage never counts) and scripts/members/profile-pilot.ts (dry run, report written OUTSIDE the repo). Numbers in constants/member-profiler.ts. No `server-only` import chain (runs from Trigger.dev and a laptop)
