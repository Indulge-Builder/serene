# The Client and Ticket Plan

> **Purpose:** the architecture for the two halves of Sia that are not built yet: the living client
> profile, and the ticketing system that replaces Freshdesk. One document, both halves, because a
> ticket is the richest thing we ever learn about a client and a profile is the thing that makes a
> ticket easy.
...
A client of Indulge is a person we serve for years. Today the memory of that person is spread across
...
We are building three things on top of the client spine that already exists (`public.clients`):
...
2. **The living client profile.** Everything we know about a client, kept as evidence-backed facts,
   summarised into one fast card, indexed for search by meaning, and scored for how happy the client
   is right now. It updates itself from every ticket, every group message, every genie note, and
   every tap in the app. Elaya reads it on both channels before she answers anything about a client.

3. **The ticketing system.** A client request becomes a ticket inside Serene: classified, attached
   to the client, pre-filled with what we already know (their addresses, their usual vendor, their
   dislikes), assigned to the right genie, moved through clear stages, watched by a small per-ticket
   agent we call the **sentinel**, and closed with the client informed. Freshdesk becomes history.
...
| Statuses | 2 Open, 3 Pending, 4 Resolved, 5 Closed (customers see "Did not solve"), 6 Nudge Client, 7 Nudge Vendor, 8 Ongoing Delivery, 9 Invoice Due (Finance group only), 9000 Assigned to AI Agent. Nudge Client, Ongoing Delivery, Invoice Due and the AI status stop the SLA timer; Nudge Vendor does not | The state machine in section 7.3 keeps every one of these meanings |
...
| Conversations | Almost entirely internal notes (417 of 417 in the July sample); client replies live in WhatsApp | Freshdesk holds the work log, WhatsApp holds the conversation. The mirror joins them |
...
- **`ticket_changes`**, append only: every field flip the sync observes (status, agent, group,
  ...
  from the webhook.
- **`client_id` on every ticket and contact**, resolved against the spine by Freshdesk contact id
...
| Which message became which ticket | `cf_periskope_message_id` where stamped; else `sia.wag_messages` in the client's group within the hour before `fd_created_at`, sender = client | The intake golden set (T3): message, ticket, category, brief, the delay from message to ticket |
| Which messages are updates to an open ticket, and which are chatter | Client messages in the group between a ticket's creation and its resolution, against the notes written right after them | The `update` and `chatter` labels of the intake exam |
| What the bishop reads and what makes them ping a genie | Notes in the internal groups ("Jokers", "Office Indulge Backend") that name a ticket number or a client, against the ticket's timeline | The sentinel's escalation rules, learned from real pings |
...
| What clients ask for, and how often | `tickets` by `client_id`, category, month | The first facts of the profile layer, and the anticipation rules of T7 |
...
### 2.4 What the member app knows about a client
...
| `member_location` | Current point plus a trail | Where the client is this week. "Dinner tonight" means Dubai, not Delhi |
| `telemetry_events` | Screens viewed, per session | Browsing interest (watches, villas). Low weight, high volume |
| `inquiries` | Product enquiries | Already flow into Serene as `lead_product_enquiries` (migration 0180). Should attach to the client, not only the lead |
...
The old Atlas `client_profiles` table held the same shape the team wants (personal, travel,
lifestyle, passions, an `elia_notes` jsonb the model wrote). Right idea, wrong storage: one mutable
jsonb per client, no evidence, no history. We keep the vocabulary and change the storage.

### 2.5 What Serene already has and reuses wholesale

| Piece | Where | Reused for |
| --- | --- | --- |
| The client spine | `public.clients` (0181), 598 clients, 207 groups mapped | Every fact, ticket and score hangs off `client_id` |
...
| The Sia UI | `/sia`, `sia-service.ts`, `SiaGroupInfoPanel` | The client card and the ticket rail render inside the panel that exists |
...
| Vendors | `vendors`, `vendor_engagements` (source `ticket` already reserved), `rankVendorsForRequest`, `find_vendors` tool | The ticket writes the engagement ledger and asks the ranker with the client's context |
...
| Redis cache-aside | `withRedisCache` | The client snapshot read |
| pgvector | Installed since 0110 | The client index |
| The budgeted API client | `freshdesk-api.ts` `FdBudget` | The posture for every future third-party pull (Zoho, the app) |

Nothing in this plan creates a second pipeline, a second brain client, a second write path, or a
second UI stack.
...
```text
  ...
  Freshdesk mirror (freshdesk.tickets + ticket_changes) │ clients            the spine        │
  Sia tickets + ticket events (this plan)               │ client_facts       truth, append-only│
  Genie notes on the client card                        │ client_health_events  live score log │
  Member app signals (wishes, saves, location, taps)    │ client_snapshot    the fast card     │
  Zoho / subscriptions (membership, money)              │ client_chunks      search by meaning │
                 │                                      └────────────────────────────────────┘
                 ▼                                                     ▲
  ...
  └──────────────┬───────────────┘                                └──────────┬───────────┘
                 │                                                           │ reports
                 ▼                                                           ▼
  Genie board · Client dossier · Bishop approvals         Elaya (proposal cards, both channels)
```
...
1. **One client, one spine.** `public.clients` is the only identity. Nothing profiles a human who is
   not a row there. Unmapped groups stay unprofiled.
2. **Truth is append only.** `client_facts`, `ticket_events`, `ticket_message_links`,
   `client_health_events`, `client_access_log`, `freshdesk.ticket_changes` are never updated or
   deleted. A correction is a new row that supersedes the old one.
3. **Derived is rebuildable.** `client_snapshot`, `client_chunks`, the ticket `summary`, the health
...
5. **The queendom is the boundary.** A client belongs to one queendom. Staff of that queendom see the
   client; admin and founder see everything. This is RLS, not a UI filter.
...
8. **Elaya proposes, humans approve, until the exam earns autonomy.** State changes that touch a
   client (a ticket created, a status moved, a message sent) start as proposals. Autonomy is granted
   per action type, by category, once precision is proven for weeks, and it is revocable by one row.
9. **The watcher never speaks.** The Baileys number reads. Whatever speaks to a client is a separate
...
11. **One schema per module.** `sia` is the Sia module's schema: the WhatsApp archive is one family
    ...
    lives in `freshdesk` and is never merged into `sia`; the client layers live in `public` beside
    the spine because Gia and Sia both read them.

---

## 5. Part A. The client twin (how client data is stored)

The founder's ask, in his words: a virtual image of the client, updating in real time from every
input, with the latest, relevant, correct data in the least time possible, and the right thing
surfaced at the right time. Atlas stored the client as two tables of key-value columns plus a
few jsonb bags. That is where every CRM starts and where every CRM gets messy: one mutable blob
per client, no evidence, no time, no way to say "this changed in July". The design below is the
fresh answer. It is one Postgres, seven stores, one loop, and every piece has exactly one rule.

### 5.1 What we actually hold about a client (the inventory)

Read from Atlas (`clients` + `client_profiles` + `profile_sources`), the Freshdesk export (the
...
| --- | --- | --- | --- | --- |
| Identity and membership | name, phones, emails, kingdom, tier, plan dates, amount, status, Zoho customer id, Freshdesk contact id, app member id, WhatsApp group | a few dozen typed columns | 1 row per client | **Store 1**, the spine |
| Simple facts | date of birth, blood group, marital status, anniversary, primary city, company and designation, social handles, "sunrise or sunset person" | one value each, sometimes changing | tens per client | **Store 2**, facts |
| Preference lists | seat, stays, cuisines, restaurants, brands, cars, watches, sports, drinks, coffee, dessert, flowers, diet, allergies, need assistance with | lists, multi-valued, contradictory over time | tens per client | **Store 2**, facts |
...
| Money | wallet, invoices, payments, renewals | typed, ledgered elsewhere | tens per client | **Store 4** (events) pointing at Zoho; never cached balances |
| Model summaries | Atlas `elia_profile`: summary, sentiment, traits, communication style, relationship strength, recurring themes, milestones | prose + lists, versioned, may be wrong | 1 per client | **Store 6**, the snapshot (rebuilt), and the facts it can be reduced to in **Store 2** |
| People and things around the client | spouse, children, house staff, the driver, favourite venues, cities, vendors they liked or refused, brands, other members they know | a network | tens of edges per client | **Store 3**, relations |
...
```text
                 STORE 1  clients (the spine)                 who; the join keys; membership
                     │
     ┌───────────────┼───────────────────────────────┐
     ▼               ▼                               ▼
 STORE 2         STORE 3                          STORE 4
 client_facts    client_relations                 client_events
 typed, append   the relationship map             the timeline: every touchpoint
 only, evidence  (client ↔ person/vendor/place/   from every source, one row each,
 confidence,     brand/interest/other client,     partitioned by month
 supersedes      strength, first/last seen)
     │               │                               │
     └───────────────┼───────────────────────────────┘
                     ▼
                 STORE 5  client_documents + client_chunks     prose + embeddings (masked)
                     │
                     ▼
                 STORE 6  client_snapshot (+ Redis)            the twin's face, versioned jsonb
                     │
                     ▼
                 STORE 7  client_health_events + client_anticipations   the score, and the right time
```
...
#### Store 2, `client_facts`: typed facts, append only
...
| Column | Meaning |
| --- | --- |
| `client_id`, `facet`, `key` | facet from the controlled vocabulary (below); key the sub-name: `address.farmhouse_alibaug`, `preference.seat`, `family.daughter` |
...
founder's examples, in `lib/constants/client-facets.ts` via `defineEnum`:
...
#### Store 3, `client_relations`: the relationship map
...
| `entity_kind`, `entity_id`, `entity_label` | `person` (a `client_people` row for family and staff), `vendor` (the vendor spine), `place` (a city or a venue, keyed by our places vocabulary), `brand`, `interest`, `client` (another member), `venue` |
...
is what the joker reads ("what would delight this family"), what the ranker reads ("this client
avoids this vendor"), and what the snapshot summarises ("closest venues: Bukhara, Zuma").

#### Store 4, `client_events`: the timeline

One row per touchpoint from every source, in one shape, so "everything that happened with this
client in March" is one ordered read.

| Column | Meaning |
| --- | --- |
| `client_id`, `occurred_at` | the partition key is the month of `occurred_at` |
...
| `actor` | `client`, `staff:<profile>`, `system`, `model` |
...
"right time" logic need one ordered stream per client, and because every source has its own
shape.

#### Store 5, `client_documents` and `client_chunks`: prose and meaning
...
digest. `client_documents` holds the unmasked text, its source pointer and its time range.
`client_chunks` holds the masked chunks with their embeddings (`vector(1024)`, HNSW cosine), the
keyword vector (`simple` config for Hinglish) and `observed_at`. Hybrid search, scoped to a
client, reciprocal-rank fused, reranked. The embedding model is Jina v4 at 1024 dimensions via
...
#### Store 6, `client_snapshot`: the twin's face

One versioned jsonb per client, rebuilt whenever truth changes (debounced 30 seconds), cached in
...
#### Store 7, `client_health_events` and `client_anticipations`: the score and the right time

The health ledger is section 5.5. `client_anticipations` is the "right time" store: one row per
...
input arrives (a message, a ticket event, a mirrored Freshdesk change, an app signal, a Zoho payment, a note)
  1. project     → one client_events row, deterministic, no model, milliseconds
  2. gate        → routing tier (Haiku): "does this carry anything about the client?" + tone
...
extractions, one weekly pass per active client; a few dollars a day.
...
| L1 | `client_facts` by facet and key, current winners | ~10 ms | "does he have a farmhouse", "what does she avoid" |
| L2 | `client_events` window by time, weighted | ~20 ms | "what happened last week", "when did they last travel" |
| L3 | `client_relations` by kind | ~10 ms | "which vendors has this family used", "who is the spouse" |
| L4 | hybrid semantic over `client_chunks`, scoped to the client, top 6 after rerank | ~200 ms | "what did she say about the Maldives villa last year" |
| L5 | the source rows themselves | ~30 ms | showing the evidence behind any answer |

The context handed to the brain for a client question is the snapshot (about 1,500 tokens) plus
...
Signals arrive from the gate (tone of client messages), the sentinel (breaches, slow replies,
...
breaches on day one so the score starts with memory. Everyone in the client's kingdom sees it,
plus admin and founder (decided 2026-09-15). The client never does.
...
| `freshdesk.tickets` (50k) | events, documents, and a one-time close pass per client for facts and relations (20 clients first, the founder checks, then all) | model |
| the WhatsApp archive | events and windows; the profiler as planned | model |

### 5.7 Why this shape, and not the others

- **Not one big JSON per client** (Atlas): no evidence, no time, no contradictions kept, and every
  writer overwrites every other. The snapshot is that JSON, rebuilt, never edited.
- **Not a graph database**: the relationship map is a few tens of edges per client; a Postgres
...
### 5.9 The Clients surface (what people see, and the first thing built)

**Decided 2026-09-15 (ten answers):** the record is the MEMBERSHIP (a couple is one client page with two names, the people under it as family); the word everywhere is **Clients** (`/clients`, table `clients`); the whole queendom (every role), admin and founder see a client; the same set may edit facts, preferences, notes, links and membership data; money shows to everyone who sees the client; the App card shows everything the app records, raw screen time included; the profiler is built later but its architecture (stores, loop) is built now; new clients come from a manual New client form for now (the won-deal hook later); WhatsApp linking is done from the client page AND from the Sia page (choose a client to link or unlink a group); Clients before tickets.

The founder's ask: a page where every client is listed with everything we
...
member, admin or founder). A **New client** form (the AddLead pattern) creates a client by hand;
...
| --- | --- | --- |
| The member list, queendom, tier, dates, amount, status, Zoho id, Freshdesk id, WhatsApp link, app member id | `export-1.csv` + `export-2.csv` | `scripts/import-clients-and-map-groups.py` extended: the two sheets become the member source of truth; existing spine rows are matched by phone, then by Freshdesk id; the mapper re-runs so the 347 invite links help the WhatsApp mapping |
| Simple facts and preferences | the Atlas profile and Typeform CSVs, the 720 Freshdesk contacts with preferences | one seed script into `client_facts` with source and date, confidence 0.9 |
| Ticket history | `freshdesk.tickets` (47,409 already linked to a client) | read live from the mirror by `client_id`; no copy |
| WhatsApp | `sia.wag_groups.client_id` | the existing mapping (207 groups) plus the coverage push |
...
M1  /clients list + dossier + New client form + link/unlink (also from the Sia page)      BUILT 2026-09-15, awaiting the first walk-through
...
Clients before tickets, because the ticket creator, the help window and the board all read
the client dossier. The profiler (T4) comes after tickets; its stores and loop are created in M0
...
| Rule | Mechanism |
| --- | --- |
| A client is visible to everyone in their kingdom, plus admin and founder (decided 2026-09-15) | RLS on every client store: `kingdom_id = get_user_kingdom()` OR role in (admin, founder), one SQL helper reading `profiles`, Rule 09 |
| Opening a client card is recorded | `client_access_log` (append only): who, which client, which surface, when |
| Chunks are never readable by users | zero user policies; only the worker and the tools read them |
| A genie's Elaya tools see only their kingdom's clients | `canAccessClient` in `lib/elaya/access.ts`, shared by both registries |
| Nothing leaves unmasked | the vault before every model and embedding call; the round-trip test in every exam. **DPDP posture (decided 2026-09-15):** the vault, the retention rules (app behaviour 180 days, everything else for the life of the membership plus the legal period), the consent record on the spine, the access log, the right-to-erasure procedure (a client's facts, events, documents and chunks deleted on request, the append-only ledgers tombstoned), and the list of external processors (Anthropic, Jina, Deepgram) are written up as one document for the security reviewer before T4 ships |
| Client text never reaches logs | the archive rule, extended |
...
1. A client writes in their WhatsApp group.
...
6. The bishop assigns a **genie**, judging workload, shift, speciality, and the client relationship.
7. The genie works the ticket: sources options, talks to vendors, writes internal notes (the whole
   thread is notes), ticks the checklist, moves the status: Open, Pending, Nudge Client, Nudge
...
8. Time triggers watch it: 30 escalation rules reassign a breached ticket to a manager after the
   ...
   when a vendor or the client has not replied. The old dashboard tagged `overdue_sync`.
9. Updates go back to the client in the WhatsApp group, typed by the genie or the bishop.
10. Money is logged in notes ("Cost Price 8250, Selling Price 8250, paid via card 1768"), in
    ...
    automatically.
11. The member app shows the client their tickets as a four-stage journey (Received, Sourcing,
...
human remembers the rest), the reason behind each assignment, the client's tone, the vendor used
(free text in notes), the money as data, and any memory that outlives the genie. What the mirror
recovers from today: the movement history, the timings, and the client join.
...
The unit is the kingdom, ideally about 200 clients each. On top, a queen; a bishop who overlooks
the work; the genies who do it; and a joker for the creative side. When a client joins, they are
...
another genie with its whole history. Serene models exactly this: `sia.queendoms` (decided 2026-09-15: the word is queendom), `profiles.queendom_id` and `concierge_role`, the client's `kingdom_id`, the group
...
The timeline the founder set: build the whole ticketing and client system now, ready to use;
...
be perfect, and nothing goes to a client without a human until the founder says so.
...
| Who reads the groups | The bishop, by eye | Elia's chatbot reads and classifies | The intake consumer reads every client message in mapped groups and classifies it; the bishop approves proposals |
...
| Context for the genie | Old tickets, asking around | Chrome extension sidebar | The client card inside the ticket, in Serene, on desktop and on the mobile layer. No extension in phase 1 |
...
| Vendor choice | Memory | Vendor score tab | `rankVendorsForRequest` with the client's preferences folded in, inside the ticket |
...
| Client updates | Typed by hand | Elia drafts, bishop taps send | Same, through the sentinel and Elaya; sending identity per DECIDE 2 |
...
| `client_id` | the spine, NOT NULL. A ticket is always for a client |
| `queendom_id` | copied from the client at creation; the RLS key |
...
| `group_jid` | the client group the request came from, when it did |
...
| `requested_for` | the date the client needs it, nullable |
...
| `closed_at`, `resolution` | `delivered`, `cancelled_by_client`, `could_not_source`, `duplicate`, `not_a_request` |
...
| Column | Meaning |
| --- | --- |
| `ticket_id`, `client_id`, `queendom_id` | denormalised so the feed needs no join and RLS is direct |
| `actor_kind` | `human`, `sentinel`, `intake`, `elaya`, `system`, `client` |
| `actor_id` | profile id when human |
| `event_type` | `created`, `proposed`, `approved`, `classified`, `assigned`, `reassigned`, `status_changed`, `priority_changed`, `brief_updated`, `checklist_ticked`, `note`, `client_message_linked`, `client_update_drafted`, `client_update_sent`, `vendor_shortlisted`, `vendor_chosen`, `quote_added`, `payment_requested`, `payment_received`, `subtask_created`, `handed_off`, `sla_warning`, `sla_breached`, `reminder_sent`, `observation`, `escalated`, `closed`, `reopened`, `learning_written` |
...
The link the founder said the model should learn first: "in this group the client sent this, and
this ticket was made for it".

| Column | Meaning |
| --- | --- |
| `ticket_id`, `chat_jid`, `wa_message_id`, `sender_jid` | the soft triple, the archive law |
| `link_kind` | `origin`, `update`, `client_reply`, `staff_reply`, `attachment` |
...
| `update_cadence_min` | how often the client must hear from us while active: 120, 240, 720, 1440 |
| `vendor_silence_min` | how long `awaiting_vendor` may sit before a nudge (Freshdesk's "Awaiting Vendor response" reminder rule) |
| `client_silence_min` | how long `awaiting_client` may sit before a gentle follow-up |
...
- Notifications: new catalog entries `ticket_assigned`, `ticket_proposed_for_approval`,
  `ticket_sla_warning`, `ticket_sla_breach_manager`, `ticket_client_replied`,
  `ticket_client_unhappy`, `ticket_daily_digest_founder`. `ticket_assigned` and
  `ticket_client_replied` are transactional, not muteable.
...
| `awaiting_client` | 6 Nudge Client | We asked the client something and wait | stopped | genie, sentinel draft | In Progress |
...
| `resolved` | 4 Resolved | Done, client informed | stopped | genie; sentinel proposes when evidence says delivered | Completed |
...
| `dropped` | (none) | Not a request, duplicate, or cancelled by the client | off | bishop, or genie with a reason | not shown |
...
`open` on a linked client message that reads as a complaint or a follow-up (Freshdesk does this by
...
2. **Intake (under 10 seconds).** The intake consumer reads the new message with the last session as
   ...
   `origin`, and a `client_facts` candidate `dislike: mushrooms (0.9)`.
...
5. **The genie.** Opens the ticket: the client card on the left, the linked messages in the middle,
...
6. **The sentinel.** Woke on every event above. It read the note, moved the summary, filled
   ...
   `next_update_due_at`, and when the genie moved to `in_delivery` it drafted the client update
   "Your Bukhara order is on its way, arriving 8pm at the farmhouse, no mushrooms." At 20:20 with
   no `resolved`, it nudges the genie: "delivery was due 8pm, any confirmation?" A client message at
   20:31 "Got it, thank you!" links as `client_reply`, tone praise, health +3, and the sentinel
   proposes `resolved`.
7. **Close.** The genie confirms. The close pass writes: the `vendor_engagements` row (Bukhara,
   dining, Alibaug, client Rahul, outcome delivered), a review prompt to the genie, the facts
...
lock so two bishops cannot double-assign. It scores every genie in the client's queendom:
...
| Relationship: handled this client in the last 90 days, and the client did not complain | medium | both ticket tables + `client_health_events` |
| Language match | low | roster + client facts |
...
1. **The rule pass, no model.** Pure code over the ticket, its policy row and the clock: first
   response due, update cadence due, vendor silent too long, client silent too long, delivery time
   ...
   wakes.
2. **The reading pass, routing tier, only when there is new text.** New linked client messages or
   new notes since the last wake. The model answers a small structured question: does this message
   belong to this ticket, what changed (a date, a count, a cancellation), what is the client's tone,
   does the client ask for a status, does anything in the brief need updating, which checklist items
   does this note complete. Output writes `brief_updated` proposals, `checklist_ticked`,
   `client_message_linked` confirmations, health events, and a refreshed `summary`.
3. **The judgement pass, reasoning tier, only when asked by rule or reading.** Draft a client
...
| Refreshing the summary | A client who sounds unhappy on any ticket |
...
| Setting SLA timestamps | A draft client update ready to send |
...
- **The face.** Both channels. A genie asks about a client or a ticket and gets an answer from the
  snapshot, the ledger, the mirror and the archive. Tools, all in the Python registry, all gated by
  `canAccessClient` and the queendom: `get_client_profile`, `search_client_history`,
  `list_client_tickets` (Sia and mirrored Freshdesk together), `get_ticket`, `list_my_tickets`,
  `get_ticket_board` (bishop and up), `find_vendors` (exists, now takes client context).
- **The hands, through the bridge.** Write tools, inline or propose-only like the lead tools:
  ...
  `update_ticket_status` (propose), `assign_ticket` (propose), `draft_client_update` (inline draft,
  the send is human), `add_client_fact` (inline, source `agent_note`, the caller's name on it).
...
1. In the Sia chat viewer, a genie or bishop selects the messages in the client's group that
   make up a request (one tap per message, or a range), and presses **Create ticket**.
2. The selected messages, the client's snapshot (addresses, preferences, dislikes, open
...
5. On the live ticket page, the **help window** on the left shows what the sentinel and the
   twin know that matters to this ticket: the client's relevant facts and dislikes, the
...
A cursor consumer named `intake` on `sia.wag_messages`, restricted to mapped client groups and
messages whose sender role is `client` (or a family member facet with permission to request). It
runs per message with the session window as context and the client's open tickets (Sia and
mirrored) as candidates.

Output, routing tier, structured:

```jsonc
{ "kind": "request | update | client_reply | question | feedback | complaint | praise | chatter",
...
`client_reply` above 0.8 write the message link and wake that ticket's sentinel; below, a link
...
proposal is compared with the mirror: did a Freshdesk ticket appear for the same client within an
...
speaks, and the founder went further: no model posts in a client group, now or as the default
...
suggested reply in the client's usual register; the agent types it in WhatsApp themselves. The
three options below stay on record; only A is planned.

| Option | How | When |
| --- | --- | --- |
| A. Human sends, Elaya drafts | Within seconds the bishop's phone gets the classification and a drafted acknowledgement in the client's usual register; the bishop pastes or edits and sends from their own number, as today | Phase 1, from tranche T3 |
| B. A speaking identity | A second WhatsApp account per queendom ("Indulge Desk") is a member of every client group; it sends acknowledgements and approved updates, never free text, always from templates or bishop-approved drafts. Separate session, separate rules, separate kill switch from the watcher | After intake precision holds at 0.9 for four weeks |
...
| Client silent in `awaiting_client` past policy | genie, with a drafted gentle follow-up | in-app |
...
| Client sounds unhappy on any ticket | bishop | push + WhatsApp |
...
Serene shows the numbers on the ticket and sums them on the client card (`counters.spend_90d_inr`).
...
| --- | --- | --- |
| Next.js on Vercel | The ticket board, the ticket page, the client card, the bishop inbox, the Sia panel additions, `/freshdesk`; every server action (Zod, `requireProfile`, the mutation cores, `{ data, error }`); the write bridge; the Freshdesk and app-signals webhooks | RLS reads through the session client; writes through the cores. No worker logic here |
...
`sia` schema, a dedicated vector store for `client_chunks`, a second worker service. Each is a lift,
...
seconds from its trigger; a Realtime board update under 2 seconds; an Elaya client answer under 4
...
| Concern | Answer |
| --- | --- |
| Who can read a client | Their queendom's staff, admin, founder. RLS on every client and ticket table, one SQL helper `get_user_queendom()` reading `profiles`, Rule 09 |
...
| What goes to Freshdesk | Nothing. Reads only, one key, one client, one budget |
| Logs | No client text, no phone, no name. Ticket numbers and ids only |
...
| Audit | `client_access_log` for every card open; `ticket_events` for every ticket change; `elaya_actions` for every proposal; `sia.extraction_runs` for every model call; `freshdesk.sync_runs` for every pull |
| Append-only tables | `client_facts`, `client_health_events`, `client_access_log`, `ticket_events`, `ticket_message_links`, `freshdesk.ticket_changes`, `freshdesk.webhook_events`: no UPDATE, no DELETE, enforced in the migration |
...
| Client-facing sends | Never from the watcher; a separate identity with templates and approvals (DECIDE 2) |
...
| --- | --- | --- | --- |
| **T0. The org and the spine** | The mirror live (migration pushed, backfill run, webhooks registered); `sia.queendoms` seeded from the three Freshdesk groups; `profiles.queendom_id` + `concierge_role` from the roster the founder supplies (the Freshdesk agents are not imported); `clients.queendom_id`, `tier`, `app_member_id` seeded from the mirror; the mapping coverage push from the Sia plan's S0; the ticket and facet vocabularies read from `freshdesk.ticket_fields` and `contact` fields; the section 2.3 queries as saved SQL | Every client with a Freshdesk ticket has a queendom. At least 85% of archive messages sit in mapped groups. At least 90% of mirrored tickets carry a `client_id`. Every genie and bishop on the founder's roster has a profile with a queendom. No model runs yet | 1 week |
| **T1. Tickets, the core** | The `sia` ticketing tables, the schema exposed to `authenticated` with RLS, the state machine, the mutation cores and RPC, the board (Realtime), the ticket page with the linked-messages view, manual create with per-category briefs and checklists, notes, the picker (bishop confirms), SLA policy rows seeded from Freshdesk's, `task_ticket_meta`, the notification categories, the client card v0 (spine + open tickets + mirrored history) inside the Sia panel | One bishop runs a full day for one queendom on Serene in shadow (creating in both) without missing anything Freshdesk has. Type check, lint, RLS tests for the queendom boundary | 2 to 3 weeks |
...
| **T3. Intake** | The `intake` cursor consumer, proposals, the Approve or Dismiss card in Serene, the WhatsApp yes or no through the resolver, the drafted acknowledgement (option A), message linking by hand in the Sia viewer, the automatic shadow comparison against the mirror | A golden set of 200 real client messages cut from the mirror join, masked and labelled: precision at least 0.90 on `request`, at least 0.85 on `update` with the right ticket, recall at least 0.85 on both. Shadow agreement with the mirror at least 90% for a week. Bishop edit rate on proposals under 20% | 2 weeks |
| **T4. The profile layer** | The vault keyed on client, `client_facts`, `client_health_events` + policy, `client_snapshot` + the profiler with the dirty queue, the seed pass over the mirror's contacts and tickets, the close pass in the sentinel, genie notes as facts, the app-signals webhook, the client card v1 with evidence links and the health score, `client_access_log` | The Sia plan's S1 and S2 exams: lossless round trip on 100 windows; facts precision at least 0.85 on the golden set; three client cards judged right by a manager. Snapshot p95 under 50 ms cached | 2 to 3 weeks |
| **T5. Memory and Elaya** | `client_chunks`, the Jina provider, the embedder, hybrid search + rerank, the client and ticket tools on both channels, context assembly with the budget | A 50-question retrieval set: recall at 5 of at least 0.8. The brain's eval suite at or above its current score with the new cases. An automated check that no model input or embedding input contains an unmasked name, phone or email | 1 to 2 weeks |
| **T6. Vendors, the app, cutover** | Ticket to engagement ledger, the ranker with client context inside the ticket, the review prompt at close, `get_member_tickets` for the app, the pilot queendom, then all three, the in-database history import, Freshdesk read only, the mirror stopped | Two weeks of one queendom fully on Serene with no return to Freshdesk. The member app shows Serene tickets with the same four stages. Every archived ticket still opens by its old number | 2 weeks |
...
| 2 | The one-minute acknowledgement | **Decided:** fully human. Serene suggests, the agent types. No model posts in a client group |
...
| 5 | The member app and Zoho | **Decided (evening):** build the feeds now, so the client card is functional and informative from day one: app signals into the timeline, Zoho wallet, invoices and payments per customer id. The member app's own ticket reads stay on Freshdesk until its later integration |
| 6 | Access: a client belongs to one queendom; the whole queendom (queen, bishop, genies, joker) sees the client and the score; admin and founder see all | **Decided.** Ticket creation from selected messages: every Sia member plus admin and founder |
...
| 8 | Who sees the health score | Every staff member who can see the client. Never the client. The reasons always shown with the number |
...
| 14 | Expired members | **Decided:** kept as client records with `membership_status = expired`, visible to their queendom (they may return); never deleted |
...
- The Notion Concierge vision: the Chrome extension is out of phase 1 (the client card inside the
...
| HNI data on more screens than before | The queendom boundary in RLS, the access log, no client text in logs, and signed URLs for everything |
