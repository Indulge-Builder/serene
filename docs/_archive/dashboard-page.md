...
Last verified: 2026-06-09 (against codebase HEAD)

## 1. Module Overview

The dashboard (`/dashboard`) is Eia's home surface: a personalised bento grid of Gia widgets. Each widget is an independently code-split client component. Summary data arrives on first paint via one server-side `get_dashboard_summary` RPC (React `cache()`), plus `getLeadVolumeByRange` (managers) / `getLeadVolumeByDomains` (admin/founder) calls merged into `initialData.lead_volume` / `initialData.lead_volume_multi`. Widgets do not share mutable state with each other.

A **global date filter** (`DashboardDateFilter`, URL-param driven) scopes the pipeline / campaign / volume data to a chosen cohort window. The range is resolved on the server from `searchParams` and threaded into every widget as a `dateRange` prop.

### Who sees what (by role)

| Role | Widgets in default layout | Data scope |
| ---- | ------------------------- | ---------- |
| `agent` | `agent-tasks`, `agent-activity` | Own tasks (`assigned_to = caller`); own activity (`actor_id = caller`). Date filter does not apply. |
| `manager` | All six widgets | Domain-scoped leads/status/campaigns/volume/cold-leads; activity on leads in `profiles.domain`; tasks still assigned to self |
| `admin`, `founder` | All six widgets | Cross-domain summary widgets; domain picker tabs on manager widgets; activity = all `lead_activities`; cold-leads = all domains |
