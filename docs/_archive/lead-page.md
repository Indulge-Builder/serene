...
- **Order:** rate limit → parse JSON → `logRawPayload` → Bearer check (marks `ingestion_error: 'unauthorized'` on the already-logged row before `401`) → `ingestLead(rawPayload, source, rawPayloadId)`.
- **Success:** `201 { leadId }`; assignment WhatsApp notifications run inside `after(notifyLeadAssigned(...))` — **not** bare `void`/fire-and-forget. The `201` is flushed to Pabbly immediately while Vercel keeps the lambda alive until `notifyLeadAssigned`'s awaited Gupshup sends settle. `notifyLeadAssigned` awaits its sends internally (`src/lib/services/lead-assignment-notify.ts`). Route exports `maxDuration = 60` so the lambda isn't killed mid-send. See root CLAUDE.md "Outward network sends" Pattern Note.
...
**Implementation (2026-06-09 verified — webhook dedup is now live):** `ingestLead()` in `lead-ingestion.ts` (step 4) **does** call dedup. When `phone` is present it runs `get_active_lead_by_phone`:

- **Active prior lead** (`new`/`touched`/`in_discussion`/`nurturing`): inserts a `duplicate_submission` activity on the existing lead, backfills `lead_id` on the raw-payload row, and returns the existing `leadId` with `is_duplicate: true`. **No new lead row is inserted.** The `IngestionResult` carries `is_duplicate`, which the webhook route forwards to `notifyLeadAssigned` as `isDuplicate`/`isNew: !is_duplicate`.
- **Terminal prior lead** (`won`/`lost`/`junk`): sets `previousLeadId = existing.id` and proceeds to insert a new lead with `previous_lead_id` linking the chain.
- **No prior lead / no phone:** normal insert path.

`createManualLead` also dedups via `get_active_lead_by_phone` → `{ duplicate: true, leadId }`.
...
### Insert payload — `attribution` snapshot + `city` extraction

The lead INSERT (`lead-ingestion.ts` step 6) builds two derived fields not present on the raw adapter output:

- **`attribution` (jsonb):** a write-once UTM/platform snapshot = `{ ...data.attribution, utm_medium, utm_campaign }`. Written exactly once at INSERT, never updated (see `COMMENT ON public.leads.attribution`, migration 0096). Absent fields are stored as `null`, not omitted, so the shape is consistent across sources. Minimum value is `{}` — never SQL `NULL`.
- **`city` (column):** extracted from `form_data.city` when present (Meta forms sometimes include it), trimmed, then **removed from `form_data`** so it isn't duplicated. `leads.city` column added in migration 0066.

### WhatsApp lead path (`createLeadFromWhatsApp`)

Called by `whatsapp-ingestion.ts` when an inbound message arrives from an unknown number. **Domain is hardcoded to `DEFAULT_LEAD_DOMAIN` (`'onboarding'`)** — WhatsApp messages carry no UTM/campaign data. `source: 'whatsapp'`, `attribution: { platform: 'whatsapp' }`. Round-robin assignment + `lead_created`/`agent_assigned` activities. Dedup is the **caller's** responsibility — this function always inserts.
...
| `assignLead` | `AssignLeadSchema` | manager / admin / founder only | Admin UPDATE lead + scratchpad null; INSERT activity | `lead_assigned` in-app notification (`.catch()` fire-and-forget); WhatsApp assignment + founder via `after(notifyLeadAssigned(...))`; SLA schedule (`.catch()`) | `{ data: { leadId }, error }` |
