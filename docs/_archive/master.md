> **ARCHIVED 2026-06-11 — docs restructure.** This file was split into
> `architecture/overview.md`, `architecture/database.md`, `architecture/auth-and-rbac.md`,
> `architecture/caching.md`, `architecture/migrations.md`, with feature sections absorbed by
> `pages/tasks.md`, `pages/leads.md` (export), `modules/gia.md` (SLA), and the integrations docs.
> Do not update this file; do not cite it. Known drift at archive time is listed in
> `_archive/README.md`.
...
>
> *Last verified: 2026-06-09 against the live codebase. Sources: changelog.md · CLAUDE.md (root + app + components + lib + migrations) · The_Rules.md · The_Gia.md · the per-page intelligence docs · `package.json` · `supabase/migrations/`*
...
9. [Migration Index — 102 Migrations](#9-migration-index--102-migrations-ordered-by-timestamp-prefix)
...
| Domain Health (founder) | ✅ Complete (2026-06-04) | Per-domain founder/admin view — lead volume, conversion, calls, revenue. `getDomainHealthMetrics` + `DomainOverviewPanel`; revenue sourced from `public.deals` (0076). Founder Performance gains an Agents/Domains two-tab shell |
| Redis cache-aside | ✅ Complete (2026-06-05) | Upstash read-through caching on leads, dashboard, performance, tasks, ad-creatives; version-counter list invalidation + dual-key lead-row invalidation; P-08 await-del-before-revalidate (see §22) |
| Export (CSV / XLSX) | ✅ Complete (2026-06-06) | Client-only `export.ts` + `export-columns.ts`; `buildXLSXWorkbook` lazy-imports SheetJS; `exportLeadsAction` feeds the table-scoped export (see §23) |
| `lead_health` removed | ✅ Complete (2026-06-08) | Built (0077–0079), reverted (0082), final removal (0083–0084) + full code/doc sweep. No column, util, strip, or filter remains (see §9 note) |
| DB hardening batch | ✅ Complete (2026-06-08) | Migrations 0085–0095 — WA-unread fix, tasks-status default, campaign first-touch key, RLS InitPlan hoist, dead-RPC-overload drops, `SELECT *` RPC fixes, archived-lead immutability, storage-RLS tightening, explicit tasks write policies |
| Lead dossier deal card | ✅ Complete (2026-06-08) | `LeadDealCard` + `getLeadDeal` surface the linked deal on `/leads/[id]`; dead `leads.deal_*` columns dropped (0097); `attribution` jsonb now written on ingestion (0096) |
...
| ----- | ------- | -------- |
| `/dashboard` | Bento grid dashboard — 6 widgets, role-scoped, RSC-first | `docs/dashboard-page.md` |
...
| `/performance` | Performance metrics — role-adaptive (agent/manager/founder) | `docs/performance-page.md` |
...
| 0083 | *(06-08 `status_counts_drop_health`)* removes the `p_health` param + `l.lead_health` predicate from `get_leads_status_counts`; drops the 9-param overload, recreates with 8 params |
| 0084 | *(06-08 `lead_health_full_removal`)* complete production removal of `lead_health` — an audit found the column, its index, `refresh_lead_health_bulk()`, and the `lead_health` SET clauses inside `add_lead_call_note` / `add_lead_plain_note` still present (0082 recorded the revert but the function bodies had drifted). Idempotent (`IF EXISTS` / `CREATE OR REPLACE`). This is the **true final** removal of the feature, not 0083. |
| 0085 | *(06-08 `fix_wa_unread_count`)* fixes `get_wa_unread_count()` — `can_access_wa_conversation()` expects a `lead_id`, but the RPC passed `wc.id` (conversation id), so the leads lookup always failed and the unread badge returned 0 for every agent. |
| 0086 | *(06-08 `fix_tasks_status_default`)* `ALTER COLUMN status SET DEFAULT 'to_do'` on `tasks` — 0017 migrated the CHECK values to `to_do…` but left the column default as legacy `'pending'`, which violated `tasks_status_check` on any INSERT that omitted `status`. |
| 0087 | *(06-08 `fix_campaign_first_touch_key`)* fixes `get_campaign_detail_metrics` `avg_hours_to_first_touch` — the lateral join read `details->>'to'`, but `update_lead_status` writes `jsonb_build_object('old_status', …, 'new_status', …)`. |
| 0088 | *(06-08 `rls_initplan_hoist`)* hoists `get_user_role()` / `get_user_domain()` into uncorrelated `(SELECT …)` InitPlan scalar subqueries across RLS policies — a STABLE function wrapped this way is evaluated once per statement instead of once per row. Covers leads, tasks, task_groups, deals, and others. |
| 0089 | *(06-08 `drop_dead_rpc_overloads`)* drops superseded RPC overloads — the old 4-param `get_dashboard_summary` (no date params) and the 2-param `get_campaign_pipeline_refresh` (no date params), leaving only the current date-aware signatures. |
| 0090 | *(06-08 `fix_select_star_rpcs`)* `get_active_lead_by_phone` rewritten with explicit column projection (no `SELECT *`). `get_personal_tasks` is intentionally left as `SELECT *` — its `RETURNS SETOF tasks` contract requires the full row type. |
| 0091 | *(06-08 `fix_leads_update_policy`)* `leads_update` RLS now requires `archived_at IS NULL` — archived leads are immutable via direct UPDATE; un-archiving must go through a SECURITY DEFINER path. |
| 0092 | *(06-08 `fix_ad_creatives_storage_rls`)* `storage.objects` INSERT/DELETE for the `ad-creatives` bucket restricted to `admin` / `founder` (inline `profiles.role` check), matching `ad_creatives` table RLS. SELECT unchanged (public bucket read). Replaces the permissive `"Ad Creative Modal insert/delete"` policies. |
| 0093 | *(06-08 `remove_duplicate_avatar_policies`)* drops the three quoted-name avatar `storage.objects` policies (`"Users can …their own avatar"`) left over from dashboard setup; the snake_case set from 0071 (`avatars_public_read` / `_insert_own` / `_update_own` / `_delete_own`) is canonical. |
| 0094 | *(06-08 `explicit_insert_delete_policies`)* explicit `tasks` write policies — `tasks_insert` (personal, self-assigned only), `tasks_delete` (agent, personal, non-terminal), `tasks_delete_privileged` (manager/admin/founder, any task). Gia-followup / group-subtask inserts remain RPC- or adminClient-only (no direct INSERT policy). `COMMENT ON TABLE public.deals` documents the intentional no-write-policy gap (all deal writes go through the admin client). |
| 0095 | *(06-08 `rls_hoist_missed_three`)* completes the 0088 InitPlan hoist for three policies it missed — `profiles_update`, `routing_config_update`, `wa_notif_logs_admin_founder_select`. The `profiles_update` WITH CHECK self-join (self-elevation guard) is preserved unchanged. |
| 0096 | *(06-08 `attribution_comment`)* `COMMENT ON COLUMN public.leads.attribution` — documents the column contract (full UTM/platform snapshot at ingestion, immutable). No data changes. Paired with the ingestion fix that now writes `attribution` on insert (0065 created the column but never wired ingestion to populate it). |
| 0097 | *(06-08 `drop_leads_dead_deal_columns`)* `ALTER TABLE public.leads DROP COLUMN deal_amount, deal_type, deal_duration`. These were dead since `public.deals` became first-class (0072–0074) — `recordDeal` writes to `deals`, never to `leads`, so the columns were always NULL on won leads. The three CHECK constraints (`leads_deal_amount_check`, `leads_deal_duration_check`, `leads_deal_type_check`) are CASCADE-dropped with the columns. Deal data on a lead is now read via `getLeadDeal(leadId)` → `LeadDealCard` on the dossier. |
| 0098 | *(06-11 `leads_search_text_trgm` — perf C-2)* `pg_trgm` extension (schema `extensions`); `leads.search_text` STORED generated column (first_name + last_name + email + city + phone) — THE search surface for every lead search path; `idx_leads_search_trgm` GIN trigram partial index (`archived_at IS NULL`). |
| 0099 | *(06-11 `status_counts_total_fold` — perf C-1)* `get_leads_status_counts` v3 (10 params): + `p_domain` (admin/founder Gia slice), + `p_going_cold` (fixes live PGRST202 — the deployed call passed a param no overload had, so the count RPC failed on every list load and pills rendered empty); search via `search_text`; `p_date_to` inclusive. Drops the 8-param overload. `totalCount` now derives from this RPC; the paginated query dropped `{ count: 'exact' }`. |
| 0100 | *(06-11 `leads_domain_created_index` — perf C-4)* `idx_leads_domain_created (domain, created_at DESC) WHERE archived_at IS NULL` — the manager list path in one index descent. |
| 0101 | *(06-11 `agent_performance_rpcs` — perf D-2)* `_agent_core_metrics()` internal helper (EXECUTE revoked from clients); `get_agent_performance(from, to, prev_from?, prev_to?)` — self-scoped (`auth.uid()`/`get_user_domain()`) jsonb with core + previous + effort + outcomes + benchmarks, replacing the agent view's ~17-query fan-out; `get_agent_roster_performance(from, to, domain?)` — role-gated per-agent SQL aggregates (manager pinned to own domain). Benchmarks become true domain-wide (previously silently reduced to the caller's own rows by agent RLS). |

> **`lead_health` is fully removed (2026-06-08).** It was built as a stored DB column + refresh job + performance health-strip + leads filter (0077–0079), then the DB column was reverted (0082), then the `get_leads_status_counts` `p_health` remnant was dropped (0083), and finally migration **0084** completed the production removal — an audit found the column, index, `refresh_lead_health_bulk()`, and the `lead_health` SET clauses inside `add_lead_call_note` / `add_lead_plain_note` still present despite 0082 (function-body drift). The code sweep deleted the `computeLeadHealth()` util, `refresh-lead-health.ts` trigger, `LeadHealthStrip` component, `getAgentLeadHealthBreakdown` service + action, the performance health strip, and the `health` leads filter. There is **no** `lead_health` column, no `computeLeadHealth()` util, no `LeadHealthStrip`, no `health` lead filter. Any reference you find to any of these is stale and should be removed. (Unrelated: **Domain Health** — `DomainOverviewPanel` / `getDomainHealthMetrics` — is a separate, live feature and was not touched.)
...
│   ├── DESIGN-DNA.md                  ← full design reference (granular token-level spec)
│   ├── design-system.md              ← design-system overview / architecture manual
│   ├── changelog.md                   ← ALL changes logged here (single source of truth)
│   ├── master.md                      ← THIS FILE
│   ├── database_architecture.sql      ← full schema snapshot (replaced full_architecture.sql)
│   ├── call-intelligence-spec.md      ← call-intelligence module spec (planned)
│   ├── decisions-to-take.md           ← open product/architecture decisions backlog
│   ├── whatsapp-notifcation.md        ← WhatsApp notification-pipeline notes
...
│   ├── performance-page.md            ← Performance module intelligence document
...
│   │   ├── dashboard/widgets/         ← 6 Gia widgets (incl. manager-cold-leads "Going Cold")
│   │   ├── deals/                     ← DealsFilters, DealCard, DealsSummaryStrip, AddDealButton, NewDealModal
│   │   ├── leads/                     ← LeadsTable, LeadsFilters, LeadDealCard, all dossier components
│   │   ├── notifications/             ← NotificationBell, NotificationPanel
│   │   ├── performance/               ← CoreFourGrid, EffortGrid, CallOutcomeBar, AgentDetailPanel, ManagerPerformancePanel, DomainOverviewPanel, DomainHealthGrid, StatAtom
...
│   │
│   ├── hooks/                         ← 9 hooks (full list + purpose in §13)
│   │   ├── useDashboardLayout.ts      ← key: eia:dashboard:layout:${userId}:v1
│   │   ├── useLeadColumnPreferences.ts← key: eia:leads:columns:${userId}:v1
│   │   ├── useDashboardCohortSync.ts  ← global dashboard date-cohort URL ↔ state sync
│   │   ├── useNotifications.ts        ← owns all notification state + Realtime
│   │   ├── useNotificationSound.ts    ← notification chime on new inbound
│   │   ├── useTaskCompletionToggle.ts ← optimistic completion-circle toggle
│   │   ├── useCreateTriggerModal.ts   ← create-trigger modal open/close + draft
│   │   ├── useDebounce.ts             ← the ONLY debounce utility
...
│
└── supabase/
    ├── migrations/                    ← 102 migrations (0001–0101; see §9 — numbering is by timestamp, two serials reused)
    │   └── CLAUDE.md
    └── config.toml
```
...
| `deals-service.ts` | `getDealsByRole` (queries `public.deals`, joins `lead(slug)` + `assignee(full_name)`), `getDealsSummary` (RPC wrapper), `getLeadDeal(leadId)` (the single non-archived deal linked to a lead — session client, RLS applies, returns `Deal` or `null`, never throws; powers `LeadDealCard` on the dossier); exports `DealWithRelations` |
...
| `leads-service.ts` | `getLeadById`, `getLeadBySlug`, `getLeadsByRole` (returns `{leads, totalCount}`), `getLeadsByRoleCached`, `getLeadFilterOptions`, `getLeadNotesFull`, `getLeadActivitiesFull`, `getCampaignMetrics`, `getCampaignDetailMetrics`, `getCampaignAgentDistribution`, `searchLeadsForTask` |
...
| `performance-service.ts` | `getCoreFourMetrics`, `getEffortMetrics`, `getCallOutcomeBreakdown`, `getPreviousPeriodCoreMetrics`, `getTeamBenchmarks`, `getAgentRosterPerformance` (manager/founder roster — revenue from `public.deals` via `won_at`), `getAgentDetailMetrics` (3 period-scoped queries), `getDomainHealthMetrics` (founder Domains tab — per-domain lead/conversion/call/revenue aggregates), `getDomainsWithLeads`; helpers `getPeriodDateRange`, `getPreviousPeriodDateRange` |
...
| `dashboard.ts` | 6 widget-refresh / range actions (all guard via `requireProfile()`): `getAgentTasksSummaryAction`, `getAgentRecentActivityAction`, `getLeadStatusSummaryAction(from?, to?, targetDomain?)`, `getLeadsByCampaignAction(from?, to?, targetDomain?)`, `getLeadVolumeByDomainsAction`, `getLeadVolumeForDomainAction`. The `*ForDomainAction` status/campaign twins were folded into the optional-`targetDomain` actions and the dead `getLeadVolumeByRangeAction` deleted (dry-audit H-5, 2026-06-11) |
...
| `leads.ts` | `addLeadCallNote`, `addLeadNote`, `updateLeadStatus`, `assignLead`, `createManualLead`, `createLeadTaskAction`, `recordDeal` (thin re-export from `deals.ts`), `updatePersonalDetails`, `updateLeadCity`, `updateLeadEmail`, `updateLeadDomain`, `updateLeadSource` (renamed from `updateLeadUtmSource` in the 0065 attribution refactor), `searchLeadsAction`, `exportLeadsAction`. *(No `updateScratchpad` — `private_scratchpad` was dropped in migration 0061.)* |
| `notifications.ts` | `markNotificationReadAction`, `markAllReadAction` |
| `performance.ts` | `getAgentDetailMetricsAction`, `getAgentSelfMetricsAction`, `getManagerRosterAction`, `getDomainHealthMetricsAction` |
...
| `dashboard-widgets.ts` | Widget registry (6 entries — adds `manager-cold-leads`), `DEFAULT_LAYOUT_BY_ROLE`, `WIDGET_MAP`, `isValidWidgetId`, `WIDGET_HEIGHT_BY_SIZE`, `WIDGET_SIZE_LABELS` |
...
| `date-range.ts` | Date-range presets for the dashboard/leads date filters (IST math imported from `ist.ts`) |
| `ist.ts` | **THE canonical IST (UTC+5:30) date math** — `IST_OFFSET_MS`, `toISTMidnight()`, `toISTEndOfDay()`, `getISTMondayStart()`, `getISTMonthStart()`, `getISTPrevMonthRange()`, `toIst()`, `istToUtc()`. Consumed by `date-range.ts`, `whatsapp-period.ts`, `performance-service.ts`, `sla.ts`, `CreatePersonalTaskModal`. Never re-fork IST boundary math |
...
| `strings.ts` | `getInitials()`, `hashString()` — THE canonical initials + deterministic colour-pick hash (avatar fallbacks, group-card accents). Never re-implement inline |
...
| 2026-06-08 | **`lead_health` feature removed entirely** (migrations 0083–0084 + code/doc sweep). | Deleted: `lead_health` column remnant in `get_leads_status_counts` (0083), the residual column/index/`refresh_lead_health_bulk()`/SET-clause drift found in production (0084), `computeLeadHealth()` util, `refresh-lead-health.ts` trigger, `LeadHealthStrip` component, `getAgentLeadHealthBreakdown` service + `getAgentLeadHealthAction`, the performance health strip, and the `health` leads filter (UI + URL param + schema + service + types). | The reverted column left dead UI/filter/RPC code querying a non-existent column. The feature was dropped from the product; the half-removed state was a correctness hazard. **Domain Health is a separate feature and was untouched.** |
| 2026-06-08 | DB hardening sweep (migrations 0085–0095). RLS policies re-evaluated `get_user_role()`/`get_user_domain()` per row; several RPCs used `SELECT *`; storage buckets had permissive/duplicate policies; `tasks.status` default and the WA-unread and campaign-first-touch RPCs carried real bugs. | Single audit pass: InitPlan-hoist all RLS helper calls into `(SELECT …)` (0088, 0095); fix the WA-unread `lead_id` mismatch (0085), the `tasks` status default (0086), and the campaign first-touch jsonb key (0087); drop dead RPC overloads (0089); explicit column projection on `get_active_lead_by_phone` (0090); archived-lead immutability (0091); tighten storage RLS (0092–0093); explicit `tasks` INSERT/DELETE policies (0094). | Per-row helper evaluation was a measurable RLS cost at scale; the rest were latent correctness bugs (unread badge always 0, INSERT default violating the CHECK, wrong first-touch metric). Defense-in-depth and performance both improved with zero product-surface change. |
| 2026-06-08 | `leads.deal_amount` / `deal_type` / `deal_duration` columns dropped (migration 0097); linked deal surfaced on the dossier instead. | Drop the three dead columns + CHECK constraints; add `getLeadDeal(leadId)` (session client, RLS-scoped, `Deal` or `null`) and a new `LeadDealCard` on `/leads/[id]`. `LeadDealCard` is a distinct display component — **not** an extension of the `DealCard` list row. | The columns were always NULL since `public.deals` became first-class (0072–0074) — `recordDeal` writes to `deals`, never `leads`. Keeping them was misleading dead schema. Deal data on a lead is read from `public.deals`, the single source of truth. |
| 2026-06-08 | `leads.attribution` jsonb now written on ingestion (migration 0096 comment + `ingestLead` fix). | `ingestLead` builds an `attributionSnapshot` (adapter bag spread over null-coalesced `utm_medium`/`utm_campaign`); minimum value `{}`, **never** SQL NULL. Written exactly once at INSERT, never in any UPDATE path. `COMMENT ON COLUMN` documents the immutable-at-ingestion contract. | 0065 created the column and backfilled history but never wired ingestion to populate it — every lead since 0065 stored NULL. `{}` = "captured, nothing present" vs NULL = "not captured" is a meaningful distinction for attribution analytics. |
...
3. /docs/master.md         ← THIS FILE — complete operational picture
4. /docs/changelog.md      ← single source of truth for what shipped, in order
5. /supabase/migrations/CLAUDE.md ← migration rules and RLS checklist
```

> *(`The_Blueprint.md`, `The_Profile.md`, `context.md`, `task-blueprint.md`, and `gia-workflow.md` are no longer in the repo — their content was folded into this master file, the per-page intelligence docs, and the changelog. Do not cite them.)*
...
> **No `ad-creatives` Redis layer.** The `campaign:ad-creative:*` key was added 2026-06-01 then **removed** (`void redis.del` after upsert/delete was a P-08 bug; the cache was dropped entirely — see changelog 2026-06-08). `ad-creatives-service.ts` is plain Supabase queries; freshness is `revalidatePath('/campaigns')` + `revalidatePath('/admin/ad-creatives')`. Do not re-add this row.
...
*Eia Master Reference — Indulge Global · Last updated: 2026-06-09*
*Sources: changelog.md · CLAUDE.md (root + app + components + lib + migrations) · The_Rules.md · The_Gia.md · all page-doc audits + live codebase verification*
