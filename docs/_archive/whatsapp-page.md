...
Last verified: 2026-06-09 (against codebase commit `fc91984`)
...
`export const maxDuration = 60` — `processInboundMessage` now awaits `notifyLeadAssigned`'s Gupshup sends inside `after()`; the lambda needs headroom so a new-number lead's agent + founder notifications complete before freeze.
...
Presence of `x-gupshup-secret` (even empty) routes to the Gupshup branch (`!== null`).

### GET

Returns Meta hub challenge when `hub.mode=subscribe`, `hub.verify_token === WEBHOOK_VERIFY_TOKEN`, and `hub.challenge` present. Otherwise plain `200` / `OK` — supports URL verification without challenge params.
...
**Gupshup `type === 'message'`:** Maps `payload.id` → message id, `+${payload.source}` → phone, `payload.source` → waId, `payload.sender.name` → senderName, builds synthetic `MetaInboundMessage` text envelope from `payload.payload.text`, runs `processInboundMessage` inside `after(async () => { ... })` (wrapped in try/catch that logs `[whatsapp/webhook] Gupshup processing error`).
...
**Meta path:** Builds a `waId → sender name` map from the `contacts[]` array, then `parseWebhookPayload` → loop: messages → `processInboundMessage` (with resolved sender name), statuses → `processStatusUpdate`. All inside `after()`.
...
Serverless handlers **terminate when the response is sent**. Without `after()`, in-flight `await` DB work (and now the awaited Gupshup notification sends from `notifyLeadAssigned`) could be frozen mid-flight. `after()` schedules continuation on the same invocation so ingestion completes **after** the fast `200` ack. Combined with `maxDuration = 60`, this keeps the lambda alive long enough for the new-lead notification chain.
...
All template sends use Gupshup `POST https://api.gupshup.io/wa/api/v1/template/msg` with `apikey` header. Each attempt is logged via `logNotification()` in a `finally` block (§4b). Template IDs live in `src/lib/constants/whatsapp.ts`.

| Event | Recipient | Template ID constant | Params (count + order) | Log `type` |
| --- | --- | --- | --- | --- |
| Lead assigned to agent | Assigned agent's profile phone | `GUPSHUP_LEAD_ASSIGNMENT_TEMPLATE_ID` (`193e330d-…`) | 3: agent first name, lead name, lead phone (or `'not provided'`) | `agent_assignment` |
| New lead → founders | Each founder with phone (parallel) | `GUPSHUP_FOUNDER_LEAD_NOTIFICATION_TEMPLATE_ID` (`d5828042-…`) | 4: `domain`, `agentName`, `leadName`, `leadPhone` | `founder_alert` |
| SLA breach → agent | Assigned agent | `GUPSHUP_SLA_AGENT_TEMPLATE_ID` (`54d5dd55-…`) | 4: `leadName`, `leadPhone`, `status`, `lastUpdatedAt` | `sla_breach` |
| SLA breach → managers | Manager profile phones | `GUPSHUP_SLA_MANAGER_TEMPLATE_ID` (`682fd320-…`) | 5: `leadName`, `leadPhone`, `agentName`, `status`, `lastUpdatedAt` | `sla_breach` |
| Agent initiates conversation | Lead's phone | `GUPSHUP_LEAD_INITIATION_TEMPLATE_ID` (`7aee2a33-…`) | 2: `leadName`, `agentName` | `lead_initiation` |
...
**Assignment call sites (all route through `notifyLeadAssigned`, all wrapped in `after()` except WhatsApp ingestion which uses plain `await` inside the route's `after()`):**
...
| `src/lib/services/whatsapp-ingestion.ts` | WhatsApp inbound new number | `await notifyLeadAssigned(...)` |

**SLA call sites:** `fireSlaBreachHandler` in `sla.ts` (agent + manager templates).
