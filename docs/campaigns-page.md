# Campaigns Page — Full Intelligence Document
...
- **Multi-video (migration 0058):** `ad_creatives_campaign_key_key` UNIQUE dropped. Many rows may share one `campaign_key`; each row is one video. Queries order `created_at DESC` (newest first). List page batch-fetches via `getAdCreativesForCampaigns`; detail page uses `getAdCreativesForCampaign`. Surfaces: `CampaignPreviewModal`, `CampaignAdCard`, lead dossier `CampaignVideoModal` (via `AdCreativeCarousel`).
...
### 3b. `get_campaign_detail_metrics` (migration `20260528000015`)
...
For each lead, a `LEFT JOIN LATERAL` subquery selects `MIN(la.created_at)` from `lead_activities` where `action_type = 'status_changed'` and `details->>'to' = 'touched'`. The average is taken across leads in the campaign/date filter. Leads never touched contribute NULL to the average (PostgreSQL `AVG` ignores NULLs).
...
| --------- | -------------------------------------- |
| `LeadsTable` | `leads`, `userId={profile.id}`, `hasActiveFilters={!!campaignName}` |
| `LeadsPagination` | `page`, `pageSize={50}`, `totalCount` — rendered only when `totalCount > pageSize` |

`LeadsTable` does not receive `filters` or `campaign` directly — filtering is entirely server-side via `getLeadsByRoleCached`. Column picker, status pills, and row rendering behave exactly as on `/leads`.
...
14. **Leads table reuse** — detail page uses `LeadsTable` + `LeadsPagination`; do not fork a campaign-only table.
