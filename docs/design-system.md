# Eia Design System — Full Reference Manual
...
**Where:** `src/app/(dashboard)/layout.tsx` — outer wrapper `className="layout-canvas min-h-screen flex"`.
...
Implemented in `src/styles/design-tokens.css` (and Tailwind utilities where mirrored):
...
| `.type-card-title` | sans, `--text-md`, semibold, tight tracking, snug leading |
| `.type-body` | sans, `--text-sm`, normal, normal leading |
| `.type-label` | sans, `--text-sm`, medium, wide tracking |
| `.type-caption` | sans, `--text-xs`, normal, loose leading |
| `.type-mono` | mono, `--text-sm`, normal, relaxed leading |
...
| `MOTION_BUTTON_DEFAULTS` | `whileTap: { scale: 0.97 }` + spring | AddLeadButton, TasksShell CTA, etc. |
...
#### `Accordion` — `Accordion.tsx` — `AccordionProps`

`single` | `multiple`. Chevron 180°. Height animate + `AnimatePresence`.
...
#### `Modal` — `Modal.tsx` + `modal.tsx` — `ModalProps`

`Modal` wraps `Dialog`. `type="lia"`: exactly Approve + Dismiss + `LiaGlyph`. Legacy `modal.tsx`: `maxWidth` string — existing callers; new work prefers `Modal.tsx`.
...
#### `ListRow` — `ListRow.tsx` — `ListRowProps`

Hover bg `--theme-paper-subtle`; chevron `x: 0 → 2`.

#### `ProgressBar` — `ProgressBar.tsx` — `ProgressBarProps`

Auto intent from value unless `intent` override. Fill animates width (Framer).

#### `EditButton` — `EditButton.tsx` — `EditButtonProps`

Pencil ghost; hover rotate -8°.
...
| ------- | ------------------------ |
| Card | `SectionCard` |
| Message bar | Spec §5.11 — not in repo |
