...
> **Last verified:** 2026-06-20 against `src/app/(dashboard)/admin/usage/page.tsx`.
...
client-side in `<UsagePresence>` (the dashboard-layout heartbeat). It is **not** "logged in":
...
alongside the page's role check. The route is not in the Sidebar's main nav for non-privileged roles.
...
**The data path:** Redis presence heartbeat (`recordPresence`, ONE `setex`, no DB write) →
1-min snapshot job `snapshotUsagePresenceTask` (appends `usage_heartbeats`, the only writer) →
...
- `UsageDashboard` (`components/admin/usage/UsageDashboard.tsx`) — `'use client'` shell; owns the
  ...
  `StatTile` strip: "Active today" (`formatDuration` of total minutes) + "People active today".
