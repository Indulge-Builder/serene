...
> **Last verified:** 2026-07-02 (stale-claim sweep: P-02 route list, P-09/Q-16 references, A-11 exception count, Q-18 cast count, A-18 exception count, D-01 rule row aligned with its own Decision Log entry; Decision Log rows appended for the shipped carve-outs). Prior full sweep 2026-06-11; repeat-offender table re-verified 2026-06-20 against `src/lib/actions/dashboard.ts` (`effectiveWidgetDomain()`; `resolveWidgetScope()` never existed).
...
| Any lead mutation (note, call, task, status, assign, deal, revive) | the cores in `lib/services/lead-mutations.ts` (`addLeadNoteCore`, `addLeadCallNoteCore`, `createLeadTaskCore`, `updateLeadStatusCore`, `assignLeadCore`, `recordDealCore`, `reviveLeadCore`); UI actions AND Elaya write tools both call the same core; never re-implement one outside |
| Voice record → transcribe cluster | `<DictationButton>`; all four voice surfaces compose it |
| Model-markdown rendering (in-app / WhatsApp) | `<ChatMarkdown>` (React) / `markdownToWhatsApp()` (`lib/utils/whatsapp-format.ts`) |
| Global domain-scope resolution (`?domain=` + cookie + role gate) | `resolveDomainParam()` in `lib/utils/domain-scope.ts` |
| Notification channel gating (per-user prefs) | `resolveChannels()` / `isChannelEnabled()` / `filterRecipientsByPref()` in `lib/services/notification-prefs-service.ts` (absence = ON, fails open; never gate `lead_initiation`/`elaya_reply`) |
...
| # | Rule |
| --- | --- |
| A-01 | Authorization reads **only** from `public.profiles`. JWT claims are never trusted for permission decisions. (`getClaims()` — in the proxy AND, since 2026-09-16, in `getCurrentProfile()` — is IDENTITY only: a locally verified signature that yields the user id, never an authorization input. The `profiles` SELECT that follows it in `getCurrentProfile()` is the authoritative check and must never be dropped for a claims-only guard. See Decision Log 2026-09-16.) |
...
| A-11 | Log and activity tables are **append-only**. No `UPDATE` or `DELETE`. Four documented narrow exceptions: `task_remarks` suppression flags (admin/founder), WhatsApp delivery-receipt status, and the two **state-machine ledgers**, `elaya_actions` (migration 0118) and `revival_candidates` (0119), whose resolve-once status flip is an admin-client UPDATE with **no user UPDATE/DELETE RLS policy ever** (the migrations' `COMMENT ON TABLE` carry the argument; 0119 reads A-11 as governing UPDATE/DELETE *RLS policies* on log tables, with admin-client state-machine flips exempt). A new exception requires a Decision Log entry first. |
...
| # | Rule |
| --- | --- |
| D-01 | No raw PII ever reaches Claude, Gemini, or any external AI model. **Live mechanism (since the 2026-06-12 Elaya foundation; see the Decision Log entry of that date):** every tool result passes `maskPii()` (`lib/elaya/pii.ts`) before serialization to the model; light mode default (phones keep last 4 digits, emails keep first char + domain), depth configurable via `elaya_settings.pii_masking_depth`. The vault (reversible pseudonymisation, vault-function-only reversal) remains the target posture and mounts at the same gateway when it lands. |
...
| P-02 | No API routes except `/api/webhooks/leads`, `/api/webhooks/whatsapp`, `/api/auth/callback`, `/api/elaya/chat` (the Elaya SSE streaming endpoint — Server Actions cannot stream; session-authenticated + rate-limited, Decision Log 2026-06-12), and `/api/manifest` (the dynamic per-icon PWA Web App Manifest: static JSON keyed off a validated `?icon=` param; the dynamic twin of `app/manifest.ts`; Decision Log entry below), and `/api/mcp` + `/.well-known/oauth-protected-resource` (the MCP connector and its OAuth discovery document, Decision Log 2026-09-19). All data mutations go through Server Actions. |
...
| P-09 | `unstable_cache` closures cannot touch `cookies()`/`headers()` — and `createClient()` from `lib/supabase/server.ts` calls `cookies()` internally, so **any service function using the session client can never be wrapped in `unstable_cache`.** Per-request memoisation uses React `cache()` instead. Where `unstable_cache` is legal, keys follow Q-16 and revalidation is `revalidateTag(tag, { expire: 0 })` (Next.js 16 two-arg form). Reference for `cache()`: `getGroupTasks` in `tasks-service.ts` (migrated off `unstable_cache` for exactly this rule; its header comment states it). **There are currently zero live `unstable_cache` call sites in `src/`.** This rule governs any future adoption. |
...
| Q-16 | **Shared cache keys (Redis and any future `unstable_cache`) include every dimension that scopes the query** — domain for domain-scoped, userId for user-scoped, **role** where the result is role-scoped. Omitting one allows cross-user/cross-domain/cross-role cache hits. See P-09 for the `cookies()` constraint and the `{ expire: 0 }` revalidation form. Live reference: the `dashboard:lead-status`/`dashboard:campaigns` Redis keys, which gained a role segment on 2026-07-02 after a manager and an admin on the same domain+range were found sharing a slot (`DASHBOARD_PIPELINE_ROLES`, `lib/constants/redis-keys.ts`). |
...
| Q-18 | **Untyped query results (joined selects, RPCs not yet in the generated types) cross into typed code only via `mapRows<TRow, TOut>()` from `lib/utils/rows.ts`.** No new `as Record<string, unknown>` **query-row** casts in services — the four legacy query-row casts (`whatsapp-api.ts` ×2, `lead-ingestion.ts` ×2) are grandfathered and that count only goes down. (`revival-gate.ts:214` is a model-JSON-output shape guard, not a query-row cast, so it sits outside this rule's target; do not cite it as licence for a new one.) "Row + joined profile" shapes use `WithAuthor<T>`/`WithAssignee<T>`/`WithActor<T>` from `lib/types`, never a fresh intersection. |
...
NEVER  omit a scoping dimension (domain, userId, role) from a shared cache key (Q-16)
...
| 2026-09-17 | AI paths (maskPii) | Every model call passes maskPii(); never raw PII to a model | TEXT still does. A FILE shown to the model (image or PDF) goes unmasked, because an image cannot be masked and the bill is the point | The vendor extractor reads invoices, and 39.2% of concierge notes carry a file whose supplier name, phone and amount appear nowhere in the text. Masking the text while sending the same details inside an image would be theatre: no privacy gained, unusable data returned. Text is protected a better way instead: phones and emails are swapped for [PHONE_1] placeholders BEFORE masking and swapped back after, so the model never sees a real number and can still say which one belongs to the vendor. Scope is narrow: the files come from Freshdesk attachments the team already shares with the account, the bytes are held in memory for one call and never stored, and the only caller is services/vendor-extract.ts | Founder (approved images 2026-09-17) |
| 2026-09-17 | R-01 (extend, never fork) | createVendorCore / logEngagementCore were manual-entry only: hardcoded identity_status 'verified', sources ['manual'], source_ref = a random uuid | Both take an optional provenance argument | A machine-extracted vendor must be born 'unverified' and a machine-logged job must carry the TICKET id as source_ref -- that is what makes the write idempotent against the (vendor_id, source, source_ref) unique key. Forking a second write path would have produced rows the /vendors form could not produce, which is the drift the cores exist to prevent | Build |
| 2026-09-18 | Ledger append-only (vendor_engagements) | closeEngagementCore was the ONE sanctioned write on an existing row | A second, narrower one: refineEngagement, reached only through logEngagementCore's provenance path -- a machine re-reading the same outside job (same vendor, source, source_ref) fills fields that are still NULL and changes nothing that is set; never outcome, never closed_at, never id | The first draft used PostgREST's upsert for a repeat. An upsert sends the whole row, so a second note that merely mentioned the vendor nulled the amount the first note's bill had supplied -- and the score reads that row -- and the fresh random id in the payload changed the row's id, orphaning any review on it (reviewer, round 2). Refinement is the honest shape of "the same job, read again": the row was never complete, and filling a NULL is not rewriting history. Outcome stays with the close, so one place decides a job is over | Build |
| 2026-09-19 | A-11 (log/ledger tables are append-only) | Two sanctioned existing-row writes on vendor_engagements: closeEngagementCore and refineEngagement. No DELETE, ever | A third, and the first DELETE: `merge_vendors` (0227) removes a ledger row when the vendor it is being merged into ALREADY holds a row for the same (source, source_ref), after folding every fact the survivor was missing out of it and re-pointing any review on it | The ledger is UNIQUE on (vendor_id, source, source_ref), so two rows for one ticket under two vendor rows cannot both survive a merge -- and that pair is not hypothetical: the extractor produced "Nitecore UAE" and "nitecore" from ticket 55146 two hours apart on 2026-09-18. Two rows describing one real job are not history, they are a fault in it. Nothing is lost: every fold is a COALESCE into the survivor, ratings follow it, and vendor_merges keeps the deleted spine row in full. Admin/founder only, service_role function, no user write policy | Build |
...
| 2026-06-05 | — | — | `public.deals` promoted to a first-class table (migrations 0072–0074), reversing the 2026-05-31 "deals = won leads" decision. `lead_id` nullable (walk-ins); `won_at` immutable; `member_id` reserved. `recordDeal` inserts a deals row before `updateLeadStatus('won')`. `get_deals_summary` rewritten over `public.deals`. | One lead has exactly one terminal `won` and cannot hold repeat/renewal deals; walk-in sales have no lead lifecycle. | — |
...
| 2026-07-02 | A-11 | Two exceptions (task_remarks suppression, WhatsApp delivery receipts) | **Retro-logged (shipped 2026-06-13/14 without log rows; this entry repairs that):** `elaya_actions` (migration 0118) and `revival_candidates` (0119) are **state-machine ledgers**: audit-trail tables with a documented resolve-once status flip (`proposed → executed/failed/dismissed`; `open → actioned/dismissed`) executed as an admin-client UPDATE. Neither has (nor may ever gain) a user UPDATE/DELETE RLS policy. 0119 also narrows A-11's reading to UPDATE/DELETE *RLS policies* on log tables; the rule row now carries all four exceptions. | Both migrations self-describe as A-11 carve-outs in their `COMMENT ON TABLE`, but per this doc's own governance a rule exception needs a log row. Appending it restores the contract. | — |
| 2026-07-02 | P-02 | Four routes (webhooks ×2, auth callback, elaya/chat) | **Retro-logged: adds `/api/manifest`**, the dynamic per-icon PWA Web App Manifest (shipped 2026-06-15 with the app-icon picker, migration 0121). Static JSON only; the `?icon=` param is validated via `isIconKey()`, never interpolated raw. The dynamic twin of `app/manifest.ts`. | A manifest must be an HTTP resource the browser can fetch by URL; a Server Action cannot serve it. Root `CLAUDE.md` already called it a sanctioned carve-out; the rule text and log now agree. | — |
| 2026-07-02 | P-09/Q-16 | `getGroupTasks` cited as THE `unstable_cache` reference; Q-16 scoped to `unstable_cache` keys with domain/userId dimensions | `getGroupTasks` migrated to React `cache()` + Redis cache-aside (it calls `createClient()` → `cookies()`, which P-09 itself forbids inside `unstable_cache`); **zero live `unstable_cache`/`revalidateTag` call sites remain in `src/`**. Q-16 generalised to every shared cache key and gained a third dimension, **role**: the dashboard `lead-status`/`campaigns` Redis keys were role-blind, so a manager and an admin on the same domain+range shared a slot; fixed by adding the rpcRole segment (`DASHBOARD_PIPELINE_ROLES`) with the invalidation side enumerating all role variants. | The named reference implementation no longer existed, and the 2026-07-02 remediation established role as a real scoping dimension the rule text lacked. | — |
| 2026-08-30 | P-02 | Five routes (webhooks ×2, auth callback, elaya/chat, api/manifest) | **Adds `/api/elaya/bridge`** — the Python brain's write bridge (service-to-service, `BRAIN_API_SECRET` bearer, timing-safe compare, fail-closed; proxy bypass like webhooks since no session exists). Ops: write-tool definitions, execute one write tool, execute a confirmed proposal. Every op re-resolves the principal from `profiles` (admin client — the parity rule) before anything runs. | The Step 3 Python brain must act without re-implementing a mutation core ("NEVER re-implement a lead mutation outside a core"): the bridge routes its writes through the SAME write-registry run() paths, cores, gates, PII seam, and `elaya_actions` ledger the Node brain uses — one mutation authority, zero drift, and Node stays the single source of the model-facing tool schema. | — |
| 2026-09-16 | A-01 | `getCurrentProfile()` (`lib/services/profiles-service.ts`) | **Identity via `auth.getClaims()`, authorization via the `profiles` row.** The per-navigation `auth.getUser()` round trip (~80–100 ms to the auth server) is replaced by the same local ES256 signature check the proxy runs; the profile SELECT that decides role / domain / `is_active` is unchanged and stays mandatory. Deactivation gains an auth-side ban (`setProfileActive` → `admin.updateUserById({ ban_duration })`, lifted on reactivation) so a deactivated session cannot refresh. | Checking on every request is right; the check should be cryptographic and local, not a network call that re-asks what the signature already proves. Revocation was already instant through the row (read per request) and is now also enforced at the auth layer. The remaining window — a user deleted at the AUTH layer (which Serene never does) holding a live token — equals the access-token lifetime; shorten it in the Supabase dashboard (60 → ~15 min) to close it. DPDP posture unchanged: access control still keys on the profile row + RLS. | — |
| 2026-09-12 | P-02 | `/api/elaya/bridge` (write bridge) | **The bridge also serves two READ tools** — `BRIDGED_READ_TOOL_NAMES` (`find_vendors`, `get_vendor_details`) ride `op=definitions` and `op=execute_tool` beside the writes, through the same dispatch and PII seam; the Python brain mirrors the names only. | The vendor ranker is the one ranking in the codebase (R-01; vendors spec: none re-rank) and spends a model call reading the request. A Python port would be a second ranker that drifts; a bridged read keeps one ranker for both brains. Every other read stays local to the Python brain. | — |
| 2026-09-16 | R-04 | The Node thinking loop: `runElayaTurn` (`lib/elaya/brain.ts`), `persona.ts`, the `node` branches in `/api/elaya/chat` and `elaya-whatsapp.ts`, the `brain_whatsapp` / `brain_in_app` rows (0179) | **FROZEN as legacy, retirement targeted 2026-10-16.** Both channels have answered from the Python brain since 2026-09-03 (WhatsApp) and 2026-09-04 (in-app); the Node loop exists only as the one-row rollback. From today no feature lands in it (the client tools of 2026-09-16 went into the shared registry both brains use, not the loop). The retirement PR deletes exactly this slice and nothing else: the loop, the Node staff persona, the two channel branches, the switch rows and `getElayaBrainForChannel`. **What stays, forever, because the Python brain runs on it:** the tool registry and `executeTool`, the PII gateway, every mutation core, the write and read bridge, the provider and adapter layer, learned memory, and every Node feature with its own model call (customer brain, revival gate, vendor request reader, client observation reader, ticket creator, sentinel). | Two thinking loops drift (the persona folds had to be ported by hand once already), but a rollback switch is cheap insurance while the new brain is young. The gate for the retirement PR: thirty days of both channels on Python with the switch never used in anger and the exam steady at or above 32 of 32. "Python thinks, Node mutates" is unchanged. | — |
| 2026-09-21 | P-02 (the `/api/mcp` row of 2026-09-19 said "founder and admin only in Phase 1; widening is one constant plus a row here") | `MCP_ROLES` (the connector's audience) | **The audience is the `mcp_audience` settings row (0233), seeded to every role but guest; `MCP_ROLES` is now the FALLBACK** when the row is missing or malformed (founder + admin, the smaller set). Read per request by `getMcpAudience()`. Nothing else widened: each role gets the toolset Elaya already gives it. | The founder asked for the whole team now and for a way to close a role without a deploy. A settings row read per call is the project's switch pattern (the brain rows, the briefing, the profiler). Fail direction is closed: a broken row shrinks the audience, never grows it. Never gate anything on the row except entry to the connector; the per-tool gates stay in the registry. | — |
| 2026-09-21 | Rule 03 / Q-13 / D-01 | `elaya_read.run()` row cap (0223 said 500) | **The analyst runner's clamp rises to 5,000 (0231) for ONE caller: the MCP connector's `export_rows`** (`exportRowsFor` in `elaya-data.ts`, founder/admin, every run logged as `export: <purpose>`). The chat tool keeps its 300 in code. Every other 0223 lock stands: the login-less role over the cleaned views only, READ ONLY transaction, one wrapped SELECT, the 8 s timeout, no contact columns anywhere. | Claude and ChatGPT analyse in their own sandboxes; 300 rows is a chat answer, not a data set. The DATABASE still decides what is reachable; only how many rows of it may leave in one call changed, for the two roles that may already see everything. Never raise it further without a row here; never let the chat tool ask for more than 300. | — |
| 2026-09-19 | P-02 | Six routes (webhooks ×2, auth callback, elaya/chat, api/manifest, elaya/bridge) | **Adds `/api/mcp`** (the Serene MCP connector: JSON-RPC over Streamable HTTP for outside AI apps) **and `/.well-known/oauth-protected-resource`** (its RFC 9728 discovery document, public static JSON, also at the `/api/mcp` suffix path). Bearer-authenticated per request through the Supabase OAuth server's tokens (`auth.getUser`, then `profiles` for the role, Rule 09); the proxy bypasses both paths like the bridge. The route publishes the principal's role-gated toolset from Elaya's registry and calls every tool through `executeTool`; it owns no tool list and no business logic. | MCP is a protocol other vendors' apps speak; a Server Action cannot serve it, and a separate service would fork the principal, the gates and the PII seam. Hosting it inside Serene makes the connector Elaya's third channel by construction (the channel-parity rule). Founder and admin only in Phase 1 (`MCP_ROLES`); widening is one constant plus a row here. Never a second tool list, never a direct database connector, never a shared key. `docs/architecture/mcp-plan.md`. | — |
| 2026-09-19 | Rule 03 / Q-13 / D-01 | Elaya's `query_database` tool (`lib/services/elaya-query-service.ts`, migration 0223) | **A model may write SQL, for founders and admins only, and only through one door.** Elaya's rule was "a tool wraps an existing service, never a direct table query". This is the sanctioned exception for the question no tool was built for. The guarantee does not rest on the prompt or on Node: the text runs AS the login-less role `elaya_reader`, which has rights on the cleaned views of schema `elaya_read` and on nothing else, inside a READ ONLY transaction, as a wrapped sub-select (one SELECT, never DML or a second statement), row-capped, under PostgREST's 8 s timeout, with `public.` and a short list of built-ins refused. The views have explicit column lists: no phone, email, password, login, raw payload or WhatsApp jid. Every query is appended to `public.elaya_query_log`; results still pass `maskPii`. | The founder asked for an Elaya that works answers out instead of depending on a tool per question. A tool per question cannot keep up with a founder's curiosity, and the risk of free SQL is containable when the DATABASE, not the model, decides what is reachable. Rehearsed on production in a rolled-back transaction: 14 attacks refused, and the role could not read one base table even with the text checks out of the way. **Hard lines:** never grant `elaya_reader` anything outside `elaya_read`; never add a view with `select *` or a contact column; never widen the tool past admin / founder without a new row here; never run model-written SQL any other way. | — |
