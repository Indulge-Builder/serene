...
| # | Rule |
| --- | --- |
| D-01 | No raw PII ever reaches Claude, Gemini, or any external AI model. Data is pseudonymized before leaving the vault; pseudonymization is reversible only by a vault function — never client-side. **Forward contract that binds the Lia build (vision: Lia v1):** nothing today sends data to an external model, and no AI integration ships before the vault exists. |
...
| P-02 | No API routes except `/api/webhooks/leads`, `/api/webhooks/whatsapp`, and `/api/auth/callback`. All data mutations go through Server Actions. |
...
| Q-08 | **Column preference hooks follow the `useLeadColumnPreferences` pattern exactly.** Same hook signature, same localStorage key convention: `eia:[module]:columns:${userId}:v1`. Column IDs are stable localStorage keys — never rename after shipping. |
...
NEVER  fork a new responsive shell/grid class — extend .eia-shell*/.eia-dossier-grid/
       .eia-board or add a variant, per D-5 (V-14)
...
NEVER  invent a new localStorage key format for column prefs —
       eia:[module]:columns:${userId}:v1 (Q-08)
...
| 2026-06-11 | S-15, S-16, D-01 | Stated as live rules | Marked **forward contracts** with the currently-enforced subset named: S-15/S-16 → S-14 + `requireProfile` gating + append-only logs today, two-actor flow later; D-01 → binds the Lia build (vision), nothing sends data to external models today. | Aspirational rules stated as fact erode trust in the enforced ones. The doc now distinguishes law from target posture explicitly. | — |
