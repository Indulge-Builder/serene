...
### Three task categories and discriminators

| Category | `task_category` | Scope |
| --- | --- | --- |
| **OS Personal** | `personal` | Individual todos; `assigned_to` is the owner; optional `tags text[]` |
| **OS Group** | `group_subtask` | Subtasks under a `task_groups` row; `group_id` FK required |
| **Gia Follow-up** | `gia_followup` | Lead-linked tasks; companion row in `task_gia_meta` required |

Shared storage: one `tasks` table plus one `task_remarks` audit thread per task.

### Design principle: accountability through narrative

Meaningful status changes should carry a remark when possible. `addTaskRemarkAction` accepts optional `statusChange` and applies it atomically via `add_task_remark_with_status` (migration 0035). `updateTaskStatusAction` remains for remark-free toggles (completion circles on list rows, header status dropdown in `SubTaskModal`).

### Two routes

| Route | Purpose |
| --- | --- |
| `/tasks` | Tabbed hub: **Gia Tasks** (GIA domains only) + **My Tasks** + **Group Tasks** (`?tab=gia\|personal\|group`) |
| `/tasks/[id]` | Full workspace for one group: list/board subtasks, Realtime, FAB add-subtask |

Sidebar: Tasks nav in `src/components/layout/Sidebar.tsx`.
...
| `status` | text | NOT NULL | `'to_do'` | CHECK: 6 values (post-0017) |
...
No INSERT/DELETE RLS on `tasks` — mutations use `adminClient` in actions with `canMutateTask` / delete rules.
...
| Allowed | Policy |
| --- | --- |
| SELECT | `task_remarks_select` — EXISTS on tasks: assignee, creator, or manager/admin/founder |
...
**RLS:** SELECT for manager/admin/founder only. No INSERT/UPDATE/DELETE policies (trigger + service role only).
...
`PERSONAL_TASKS_PAGE_SIZE = 50`. On RPC error: logs + empty result (TD-002).
...
- **Idempotency key (exact):** `` `task-reminder-${taskId}` ``
- Passes to `tasks.trigger('send-task-reminder', { taskId, assignedTo }, { delay: dueAt, idempotencyKey, tags: [`task-reminder-${taskId}`] })`
- **No-op** if `dueAt <= now()` — never errors on past dates
- Tag: `task-reminder-${taskId}` for cancel lookup

### `cancelTaskReminder(taskId)`

- Lists runs with tag + status `DELAYED` \| `QUEUED`; cancels all
- **`deleteTaskAction`:** awaited cancel **before** delete — delete aborts if cancel fails

### Deduplication guarantee (no `reminder_run_id` column)

Trigger.dev v3 deduplicates by idempotency key for non-terminal states including `DELAYED`. A second `scheduleTaskReminder` while a DELAYED run exists returns the existing run (`isCached: true`) — no orphaned duplicate. Documented in comment block at top of `task-reminders.ts`.

### On fire: `sendTaskReminderTask`

- Creates notification `type: 'task_due'`, `action_url: '/tasks'`, title `"Task due now"`
...
**Not** `PersonalTasksTab.tsx` (legacy priority-section layout; **unmounted**; do not document as active UI).
...
- Manager+: `listAgentsForDomain` for assignee picker
...
14. **`adminClient` justification:** No INSERT/UPDATE RLS on `tasks`/`task_groups`; `adminClient` bypasses RLS so the application layer must enforce the same access rules RLS would (`canMutateTask`, role checks, view = post gate). Never skip the pre-write auth fetch.
...
18. **`GroupTasksTab` agents:** `listAgentsForDomain` called once at tab level — `GroupRow` must never call it per group.

19. **`AnimatePresence`:** Required at **call site** for `SubTaskModal` and `CreateGiaTaskModal` — exit animation does not run if omitted.

20. **My Tasks vs legacy:** `MyTasksCalendarView` is the active My Tasks UI. `PersonalTasksTab` is legacy and unmounted — do not copy its COMPLETED accordion into the calendar view.
...
| `PersonalTasksTab.tsx` | **Legacy — unmounted** |
