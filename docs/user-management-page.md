...
| `updateProfileFields` | `id`, partial non-auth fields | `{ data, error }` | session | `updateProfile`, `updateProfileAvatar`, `createUser` (phone/job_title) |
...
- **Tree:** `<main>` → header (`Team.` + page-title-dot) + `Add Member` link → `<UsersTable users={users} />`
...
- **Filter bar chrome:** `SlidersHorizontal` + **active-count badge** (accent pill) when search/role/domain active
- **Display:** Card list (`UserCard`), not `Table<T>` despite historical refactor notes — each card links to `/admin/users/[id]`
...
- Fields: full_name, phone, job_title, username
- Labels: `label-micro` style (`--text-2xs`, widest tracking, tertiary uppercase)
- Action: `updateProfile` (phone normalized server-side)
...
7. **last_seen_at:** Spec — middleware updates on authenticated requests, max once per minute per user, for online indicators. **Not implemented in `src/proxy.ts` / middleware in repo as of 2026-06-01**; column exists for future use.
