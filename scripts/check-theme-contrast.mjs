...
try {
  ...
  const expression = `(() => {
    ...
    for(const theme of ['earth','air','water','fire','candy','rose','moss','lilac'])for(const mode of ['light','dark']){
      ...
      for(const el of document.querySelectorAll('.serene-field-control:not(:disabled)')){const css=getComputedStyle(el);const backgrounds=css.backgroundImage.match(/rgba?\([^)]*\)/g)??[css.backgroundColor];for(const bg of backgrounds){pairs.push(['field ink / gradient endpoint',css.color,bg,4.5]);pairs.push(['field focus / gradient endpoint','var(--neu-focus-edge)',bg,3]);pairs.push(['field error / gradient endpoint','var(--color-danger-text)',bg,4.5]);}}
