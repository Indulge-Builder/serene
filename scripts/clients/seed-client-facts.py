#!/usr/bin/env python3
"""
scripts/clients/seed-client-facts.py — seed STORE 2 (public.client_facts, migration 0194) from
what Atlas, the onboarding forms, the Freshdesk contacts and the member sheets already know.

Sources → facts (client-ticket-plan.md 5.6), each row carrying source, confidence, evidence,
observed_at, and the client resolved by Atlas client id → phone, or by phone alone:
  cleint-data/atlas-client-data.csv       Atlas clients: phone → client; notes → facet note
...
Idempotent: a fact is skipped when the same (client, facet, key, value, source) already exists.
DRY RUN by default: prints the counts per facet and source. --apply writes in batches.

Usage:
  set -a && source .env.local && set +a
  python3 scripts/clients/seed-client-facts.py            # dry run
  python3 scripts/clients/seed-client-facts.py --apply
"""
...
def facts_from_fields(fields: dict, client_id: str, source: str, confidence: float, observed_at: str | None, evidence: dict):
    out = []
    for name, raw in fields.items():
        ...
        for val in as_list(raw):
            out.append({
                "client_id": client_id, "facet": facet, "key": key, "value": val, "polarity": polarity,
...
def main():
    ...
    clients = get_all("clients?select=id,primary_phone,alt_phones,freshdesk_contact_id,full_name")
    ...
    print(f"clients: {len(clients)} ({len(by_phone)} phone keys, {len(by_fd)} Freshdesk ids)")

    facts: list[dict] = []

    # 1. Atlas: clients.csv gives atlas id → phone; profile.csv hangs off the atlas id.
    atlas_clients = load_csv(f"{DATA}/atlas-client-data.csv")
    atlas_to_client: dict[str, str] = {}
    for r in atlas_clients:
        cid = by_phone.get(last10(r.get("phone_number")) or "")
        if cid:
            atlas_to_client[r["id"]] = cid
            if clean(r.get("notes")):
                facts.append({"client_id": cid, "facet": "note", "key": "", "value": clean(r["notes"]), "polarity": "neutral",
                              "source": "atlas", "confidence": 1.0, "evidence": {"atlas_client_id": r["id"], "field": "notes"},
                              "observed_at": r.get("updated_at") or datetime.now(timezone.utc).isoformat()})
    print(f"atlas clients matched: {len(atlas_to_client)} of {len(atlas_clients)}")
    for r in load_csv(f"{DATA}/altas-cleint-profile.csv"):
        cid = atlas_to_client.get(r.get("client_id", ""))
        if not cid:
            continue
    ...
    n_fd = 0
    contacts = get_all("contacts?select=id,client_id,custom_fields,fd_updated_at&client_id=not.is.null", schema="freshdesk")
    for c in contacts:
        cf = c.get("custom_fields") or {}
        new = facts_from_fields(cf, c["client_id"], "freshdesk_contact", 0.9, c.get("fd_updated_at"),
        ...
        for r in load_csv(fname):
            ...
            for col_name, key in ((city_col, "primary_city"), (company_col, "company")):
                if col_name and clean(r.get(col_name)):
                    facts.append({"client_id": cid, "facet": "identity", "key": key, "value": clean(r[col_name]), "polarity": "neutral",
    ...
    deduped = []
    for f in facts:
        k = (f["client_id"], f["facet"], f["key"], f["value"].lower(), f["source"])
        if k in seen:
            continue
        seen.add(k)
        deduped.append(f)
    existing = set()
    try:
        for row in get_all("client_facts?select=client_id,facet,key,value,source"):
            existing.add((row["client_id"], row["facet"], row["key"], (row["value"] or "").lower(), row["source"]))
    except Exception as e:  # before migration 0194 the table does not exist
        print(f"  ! client_facts not readable yet ({str(e)[:60]}); nothing is 'already present'")
    todo = [f for f in deduped if (f["client_id"], f["facet"], f["key"], f["value"].lower(), f["source"]) not in existing]
    ...
    print("  clients touched:", len({f["client_id"] for f in todo}))
    if not args.apply:
        print("\nDRY RUN — nothing written. Re-run with --apply.")
        return
    for i in range(0, len(todo), 500):
        rest("POST", "client_facts", todo[i:i + 500], prefer="return=minimal")
