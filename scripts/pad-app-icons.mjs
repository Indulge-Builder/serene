 ...
 *
 * The default home-screen icon is the mark on a SOLID white plate (2026-09-26, the
 * founder's pick: the same on iPhone and Android, never the black an iPhone paints
 * behind a transparent icon); the decorative picks stay on the cream plate (#ECE8E1,
 * NEU_CANVAS_LIGHT). A solid plate also keeps the manifest's `maskable` entry valid
 * (Android crops a circle/squircle; corners stay filled). NEVER re-add maskable if an
 * icon ever reverts to a transparent background.
 *
 * Outputs:
 ...
 *   - public/logo-bg-removed.webp       the transparent mark the sidebar, mobile
 *                                       trigger and auth pages show (240)
 *   - src/app/favicon.ico               16/32/48/64, the transparent mark (no plate,
 *                                       2026-09-26), a heavier stroke so it reads at 16
...
// NEU_CANVAS_LIGHT #ECE8E1 (lib/constants/appearance.ts) — the decorative picks' plate.
const PLATE_HEX = "#ECE8E1";
const PLATE = { r: 0xec, g: 0xe8, b: 0xe1, alpha: 1 };
// The default icon's plate: white, the founder's pick for the phone shortcut.
const MARK_PLATE_HEX = "#FFFFFF";
const MARK_PLATE = { r: 0xff, g: 0xff, b: 0xff, alpha: 1 };
...
const plateFor = (key) => (key === DEFAULT_KEY ? MARK_PLATE : PLATE);
const plateHexFor = (key) => (key === DEFAULT_KEY ? MARK_PLATE_HEX : PLATE_HEX);

async function buildPlated(key) {
  const glyph = await glyphFor(key);
  const plate = plateFor(key);
  // Composite the glyph onto a SOLID plate — no transparency in the output, so
  // the OS shows our plate, never its own fill.
...
for (const key of KEYS) {
  ...
  console.log(`built ${key}.webp → ${SIZE}×${SIZE} on ${plateHexFor(key)}, glyph ${inner}px (inset ${pad}px)`);
}
...
// logo-bg-removed.webp — the transparent mark at 240 with the old asset's margin (~200px mark).
{
  ...
  await sharp({ create: { width: 240, height: 240, channels: 4, background: { r: 0, g: 0, b: 0, alpha: 0 } } })
    ...
    .webp({ quality: 95 })
    .toFile(join(OUT_DIR, "logo-bg-removed.webp"));
  console.log("built public/logo-bg-removed.webp → 240×240, transparent");
}

// favicon.ico — the transparent mark (no plate, 2026-09-26: the tab shows the rings
// alone), a heavier stroke so it reads at 16px, scaled so the outer rings (radius 92
// + half the stroke) just fit the box. Every size a PNG entry (the ICO container,
// written by hand: no dependency).
{
  const faviconSvg = (px) =>
    `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200 200" width="${px}" height="${px}">` +
    `<g transform="translate(100 100) scale(1.03) translate(-100 -100)">${markCircles(9)}</g></svg>`;
  const entries = [];
  ...
  console.log("built src/app/favicon.ico → 16/32/48/64, transparent");
}
