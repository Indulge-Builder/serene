...
**`elaya-presence` rules:** the **live `/elaya` chat, shrunk into the widget** — not a teaser, not a modal. The card resolves the user's single active conversation on mount via `getElayaChatSeedAction` (`lib/actions/elaya.ts` — a `'use client'` widget can't call `elaya-service` directly, A-15; the SAME seed `/elaya` resolves, R-01) and renders the shared `ElayaChatShell` in `embedded` mode (flush, chat-only — no card-in-a-card, no identity rail) filling the widget box. Lazy `next/dynamic` import keeps the chat bundle out of the dashboard route chunk. Until the seed lands, the breathing `LiaGlyph` holds the seat (a static glyph = Elaya absent — never that). The user says hi and gets a reply **inside the widget** — the SSE loop, transcript, cap, and voice all live in the one shell; never re-inline a composer or any chat plumbing on the card. No three.js (that lazy-loads post-Elaya-ship).
