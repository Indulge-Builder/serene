...
**Layout:** full-height two-panel, **flex-fill (not `dvh`/percentage math)**. The page `<main>` is `flex min-h-0 flex-1 flex-col overflow-hidden`; `WhatsAppShell`, its right pane, and `ConversationPanel` all use `flex: 1; minHeight: 0` — never `height: 100%` (matches the Elaya/leads/deals convention). No `p-8` page padding — the shell is flush. `WhatsAppShell` owns both panels.

**Mobile header alignment (full-bleed carve-out, fixed 2026-06-17):** because `/whatsapp` is full-bleed (its `<main>` has no `p-4` ladder), the global `.serene-shell-paper main.p-4` top-offset rule — the one that drops every other page's `<h1>` onto the floating nav hamburger's line (`top: --space-7 + safe-area`) — **skips it**. Without a replacement, the rail "WhatsApp" title (and the single-pane conversation back-header) sit ~12px too high and ignore the notch. Two `< --bp-md` rules in globals.css restore it: `.serene-shell-paper .serene-wa-rail` (rail top padding → `--space-7 + safe-area-top`) and `.serene-shell-paper .serene-wa-pane-header` (same top offset + `padding-left: 68px` so the back arrow clears the overlaying hamburger; applied only when `onBack` is set = single-pane mobile). The `52px` `.type-page-title` left-indent already reaches the rail title (it's a `.type-page-title` in the gutter). If you add a title row or change the trigger geometry, update these together.
