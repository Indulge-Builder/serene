...
import { SIA_GROUP_PARAM, SIA_MESSAGE_PARAM } from "@/lib/constants/sia-roles";

export const metadata = { title: "Sia" };
...
export default async function SiaPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  // ?message=<wa_message_id> (siaMessageHref): once that chat is open, scroll to this message. Only with a valid group.
  const wantedMessage = params[SIA_MESSAGE_PARAM];
  const initialMessageId = initialGroupJid && typeof wantedMessage === "string" && wantedMessage.length > 0 && wantedMessage.length <= 120 ? wantedMessage : null;

  return (
    <main className="flex-1 min-h-0 flex flex-col p-4 sm:p-6 lg:p-8">
      <SiaWorkspace canManage={scope.kind === "all"} groups={groups} initialGroupJid={initialGroupJid} initialMessageId={initialMessageId} />
    </main>
  );
}
