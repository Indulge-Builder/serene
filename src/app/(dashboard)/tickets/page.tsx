...
function parseFilters(sp: Awaited<SearchParams>): TicketListFilters {
  ...
  return {
    ...
    tag: getString('tag'), mine: getString('mine') === '1', page: Math.max(1, parseInt(getString('page') ?? '1', 10) || 1),
  };
}
...
async function TicketsAsync({ filters, callerId }: { filters: TicketListFilters; callerId: string }) {
  ...
  const hasFilters = Boolean(filters.search || filters.status.length || filters.queendom || filters.assignee || filters.category || filters.tag || filters.mine);
  const settings = await getTicketSettings();
  return (
    <>
      <TicketsTable tickets={tickets} hasFilters={hasFilters} labels={settings.statusLabels} />
