...
| `(dashboard)/freshdesk`, `(dashboard)/freshdesk/[id]` | `docs/integrations/freshdesk.md` — the Freshdesk mirror (admin/founder; the page gate is the trust boundary, the `freshdesk` schema has no user policies). List: overview strip (0196, same filters as the table) + FilterBar + dense table + Pagination + Sync now; `?client=<uuid>` scopes everything to one client (the client page's "See tickets"); dossier: thread, movement timeline, summary card. Display-only; Serene never writes to Freshdesk |
| `(dashboard)/clients/[id]/finance` | the client's money page: the membership tiles + movements Serene holds, then the LIVE Zoho part streamed in (`ClientZohoCards`: balances, the customer record, invoices, payments, credit notes; `getClientFinance`, 1-minute Redis copy, Refresh); RLS + `canSeeClientFinance`; every open logged `finance_page` |
...
| `(dashboard)/clients`, `(dashboard)/clients/[id]` | `client-ticket-plan.md` 5.9 — the client twin's surface (the whole queendom + admin/founder; rows RLS-scoped by `client_visible`). List: FilterBar + dense table + Pagination + New client; dossier: one card per store, every open logged to `client_access_log` |
