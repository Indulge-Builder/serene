...
| `CardHeader` (leads/) | `leads/CardHeader.tsx` | inline props | **THE dossier card-header strip** (dry-audit D3) — icon (0.875rem tertiary) + uppercase micro-label + optional `right` slot on `--theme-paper-subtle` with the `--space-4/--space-5` padding + border-bottom. All 7 dossier cards compose it (`style`/`iconStyle`/`labelStyle` escape hatches for the accent variant + active-icon tint). Server-component-safe. Never re-inline a dossier header. |
