...
| `MessageBubble`          | `MessageBubble.tsx`          | Single message. Inbound (`--neu-surface-high`) / outbound (`--neu-chat-user-bg` — THE chat sender-bubble token, same contract as `ElayaMessageBubble`). Delivery icons (sent = single tertiary, delivered = double tertiary, read = double `--neu-info-deep`). Bot label. **Media:** `MediaPlaceholder` renders an inline `<img>` thumbnail (≤240px, click→open) for images and `<video controls>` for videos (caption beneath); document/audio + any url-less media show the labelled chip + View link. A non-media message with blank content renders a muted serif-italic **"Unsupported message"** placeholder (never a blank bubble). |
