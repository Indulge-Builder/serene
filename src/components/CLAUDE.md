...
| -------------- | ------------------ | --------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `SeedMandala`  | `SeedMandala.tsx`  | `SeedMandalaProps`                      | **THE procedural brand mark** (logo-motion handoff 2026-07-03). 8 stroked circles, r=46, centers on a ring of radius 46 (offset === r — every circle passes through the exact center; that crossing IS the 8-petal seed flower) **plus the intersecting centre circle r48** (2026-09-25: the official Indulge logo's ninth ring, as the Indulge app draws it; it draws LAST, sealing the mark). Variants: `gradient` (umber→gold, stroke 2.2, default), `currentColor` (stroke 7, tiny in-control sizes), `darkDisc` (light gold, stroke 4 — charcoal/scrim grounds). `draw` (1.15s trace, 90ms stagger, centre circle at 720ms) + `spin` (s/rev — FIXED speeds: boot 24 (the Indulge app's mandalaSpin, starting once the draw completes) · spinners/buttons/veil 3.5 · Elaya 8 · resting marks 120; never faster than 3.5). Brand gradient stops are the ONLY sanctioned component hex — never theme-tinted. Spin/draw are class-driven (`.serene-logo-spin`/`.serene-logo-draw`) so reduced-motion kills them (mark rests finished). The raster copies (app icons, logo-bg-removed.webp, favicon) are rendered from the same geometry by `scripts/pad-app-icons.mjs`; never redraw them by eye.
| `LogoSpinner`  | `LogoSpinner.tsx`  | `LogoSpinnerProps`                      | **THE loading indicator — the arc `Spinner` is DELETED, never recreate it.** Seed mandala at 3.5s/rev. Sizes: `lg` 38px mark in 56px inset cream well, `md` 27-in-40 well, `sm` 24 bare. Wells: `--neu-surface` + `--neu-shadow-inset`. Tiny in-control indicators (save chips, composer send) compose `<SeedMandala variant="currentColor" spin={3.5}>` directly at 14–16px instead.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `Button`       | `Button.tsx`       | `ButtonProps`                           | Variants: primary/secondary/ghost/danger/success. Sizes: xs/sm/md/lg. Border-radius `--radius-sm` (never `--radius-md`, §5.01). Loading state swaps `iconLeft` slot for an 18px `currentColor` `SeedMandala` at 3.5s/rev, sets `cursor: wait`, drops primary to opacity 0.85; optional `loadingLabel` swaps children for a progressive verb ("Saving…"). `iconLeft`/`iconRight`. Primary: bg `--theme-accent`, fg `--theme-accent-fg`, resting `--shadow-accent-glow`, hover `--theme-accent-hover` + `--shadow-accent-lift` + `translateY(-1px)`. Secondary: paper-subtle + `--theme-paper-border` + `--shadow-1`, hover border `--theme-accent-muted`. Ghost: transparent + `--theme-text-primary`, hover bg paper-subtle. Danger/Success: soft `-light` rest → saturated base + `--theme-text-inverse` on hover (intentional drift from design-dna.md §5.01 saturated default — preserved to avoid breaking existing consumers). Disabled: `opacity 0.5`, `pointer-events: none`, `cursor: not-allowed`. Uses `React.forwardRef` — required by `MotionButton`. **No `whileTap` here** — Button stays a plain `<button>` so non-animated callers pay zero Framer Motion bundle cost; tap-scale lives in `MotionButton` + `MOTION_BUTTON_DEFAULTS`. **Variant chrome (rest + hover) lives in the `.serene-btn-*` classes in `design-tokens.css`** (design-audit 2026-06-11): `:hover` is gated to `(hover: hover) and (pointer: fine)` so a tap never leaves a sticky hover state — never reintroduce JS `onMouseEnter` hover styling. Keyboard focus is the ONE shared outline (`--neu-focus-edge`, 2px, 2px out; `:focus-visible` only, so a mouse click never shows it) — the old `.serene-pressable` box-shadow ring and the `suppressFocusRing` prop were removed 2026-09-25 (control-system.md, "Focus and state without rings"). **Press feedback is CSS-only:** every Button carries the `.serene-pressable` class (`design-tokens.css`) — `:active → scale(0.97)`, declared after the hover rules so it wins by cascade (no `!important`), reduced-motion-gated; never add a second press mechanism to a Button consumer. |
| `CardHeader` (leads/) | `leads/CardHeader.tsx` | inline props | **THE dossier card-header strip** (dry-audit D3) — accent icon (0.875rem) + accent `.label-micro` label + optional `right` slot on the THEMED `--neu-header-wash` (22% accent mixed into the surface — the 12% `--theme-accent-surface` read washed-out; header intensity is tuned ONLY via that token in serene-neumorphic-tokens.css) with a `--neu-header-edge` hairline border-bottom and the `--space-4/--space-5` padding — the header is the theme-coloured zone of a card (promoted to default 2026-07-03; never `--theme-paper-subtle` — the sunken well tone, neu Rule 4). All 7 dossier cards compose it (`style`/`iconStyle`/`labelStyle` escape hatches for the accent variant + active-icon tint). Server-component-safe. Never re-inline a dossier header. |
...
| `SearchBar`    | `SearchBar.tsx`    | `SearchBarProps`                        | Controlled. Sizes: sm/md/lg. Default placeholder `"Search"`. Clear button (hover → `--theme-text-primary`, `var(--transition-hover)`). Focus: the field frame (`--neu-focus-edge` border + a 1px ring of the same, hugging; never the offset outline). `caretColor: --theme-accent`. Placeholder colour resolved via `.serene-input` class (`::placeholder` unreachable from inline style). Fires `onChange` every keystroke — debounce by consumer.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `Checkbox`     | `Checkbox.tsx`     | `CheckboxProps`                         | **THE small themed tick box** (2026-09-25, promoted from LeadsTable's private CheckboxCell). A real `<button role="checkbox">`: a wrapping `<label>` toggles it, Space and Enter toggle it, the shared keyboard outline draws on focus; `indeterminate` draws the mixed dash; `size` 16 (tables) or 18 (forms); `label` renders the box + words row, else pass `aria-label`. Accent fill + accent ink when on, the field material when off. Used by the leads table (select all / row), notification preferences, the SLA channel picks. A done-state tile is `CheckTile`; on/off settings are `Toggle`; a radio row keeps its native input visually hidden (`sr-only`) behind a drawn dot (`TaskTypeField`, `.serene-radio-row` for the row focus). Never a native `<input type="checkbox">`; `:root { accent-color }` in the neu tokens is only the safety net. |
| `FormSelect`   | `FormSelect.tsx`   | `FormSelectProps`                       | **THE form select** (a FilterDropdown in its `field` look, portaled, keyboard-operated, with type-ahead). Takes plain `<option>` children, `<optgroup>`s (each group's label becomes a micro heading in the list via `FilterDropdownItem.group`, 2026-09-25) and disabled options. `name` registers the value with a plain `<form>` post through a hidden input (RoleDomainFields' domain and queendom_id); the current value rides as the trigger's description so a screen reader hears it. Used by every form select since 2026-09-25 (Won deal category, role/domain/queendom, subscription type and currency, top-up and recharge). Field's native `Select` stays only for multi-selects. |
...
**`Carousel` (`Carousel.tsx`) — THE generic swipeable deck primitive** (Phase 5). Generic over `T` with a `renderItem` slot; **controlled** `index` + `onIndexChange` (the consumer owns active state). Horizontal track on a motion value, sprung on `SPRING_CONFIG` — transform/opacity only (Never-Do list). **The swipe follows the finger 1:1** (2026-09-25: Framer `drag="x"` with direction lock, `dragElastic` rubber-banding at the ends, `dragMomentum` off): on release the landing slide is chosen from where the flick would come to rest (Apple's `(v/1000)·d/(1−d)` projection, `d = 0.998`), one slide per gesture, and the settle spring starts at the finger's velocity, so drag and settle read as one motion. A vertical scroll inside a slide is never hijacked (each slide is `overflowY:auto`, `touch-action: pan-y`); hidden slides are `inert`. The slide width is measured before paint (`useLayoutEffect` + ResizeObserver), so a deck opened on slide 3 lands there at once. A single **controls bar below the viewport** (prev arrow · dot indicator + `n / total` counter · next arrow — arrows are quiet `paper-subtle` pills, disabled at the ends; they are NOT overlaid on the slide), ←/→ keyboard nav. Display-only (A-06). **This is NOT `campaigns/AdCreativeCarousel`** — that one is video-coupled (`AdCreativePlayer` + `AdCreative` type) with no touch support; this is content-agnostic. The two coexist by design (R-01 — the video coupling justified a separate generic primitive over an extraction). Sole consumer today: `FounderDrillDownDeck` (the `/performance` founder deck).
...
| Component       | File                | Props Interface      | Notes                                                                                                |
| --------------- | ------------------- | -------------------- | ---------------------------------------------------------------------------------------------------- |
| `Table`         | `Table.tsx`         | `TableProps<T>`      | Generic. Sticky header option. Dev-only `console.warn` when `rowCount > 100 && !virtualized` (P-03). `previewRows?: number` — THE built-in P-03 "paginated approach": renders the first N rows + a "Show all N" expander row (one-way client reveal; suppresses the warn). Adopted by the three `/escalations` sections (`SECTION_PREVIEW_ROWS = 50`). |
| `EmptyState`    | `EmptyState.tsx`    | `EmptyStateProps`    | **THE canonical empty state — ONE anatomy everywhere** (2026-09-25, modelled on the /tickets table empty): a raised tile, a Playfair italic title (balanced lines), a sans tertiary description (pretty), at most one `action`, centred, Framer entrance. The tile holds `icon` or, without one, the Serene mark (static in `inline`, a 120s turn in `hero`). `variant="hero"` (default with an icon) = a page, table or section: 64px tile, xl title. `variant="inline"` (default without) = inside a card, panel, modal or mobile room: the same anatomy at 44px (it used to be a bare italic line). `framed` (hero only) = the page-level paper card the tickets empty sits in; use it when the empty state IS the page's content and nothing else holds it (it replaced the hand-written wrapper cards on Tickets, Members, Freshdesk, Vendors, Performance). **Icon rule:** a page with nothing in it takes its sidebar icon (Members `Users`, Deals `Trophy`, Notes `NotebookPen`, …); an all-clear (Escalations, notifications "All caught up", the error log) takes the mark, never a warning icon; elsewhere the mark. Menus and listboxes (FilterDropdown, the command palette, pickers) keep a short text line — they are controls, not screens. The old `brand`, `ambient` and `size` props are gone. **Server-safe (2026-09-26):** no `'use client'`, so a server page can pass `icon={Trophy}` (a component cannot cross into a client component: "Only plain objects can be passed to Client Components"); the icon renders on the caller's side and only the fade-in is client (`EmptyStateEntrance`, private). Never add hooks or `'use client'` back. Never hand-roll an empty state or wrap one in another card. |
| `StatTile`      | `StatTile.tsx`      | inline props         | **THE labelled stat tile** (dry-audit L-8). `variant: 'card'` (paper card chrome, micro label over `--text-2xl` semibold value, optional coloured `sub` line — campaign metrics strip) or `'cell'` (bare centred cell, accent value over micro label — composed inside a `StatStrip`; `size="sm"` = the breakdown cell: lg primary value, optional `dot`, one-line sentence-case legend, optional `sub`, 2026-09-25). **Both variants' value is `--font-mono` + tabular-nums** (number-font rule, 2026-07-06 — was Playfair). Server-component-safe. Performance `MetricCard` deliberately stays bespoke (delta/sparkline/motion decoration); any NEW plain stat tile composes this. |
| `StatStrip`     | `StatStrip.tsx`     | `{ title? aside? footer? divided? children }` | **THE strip card of `StatTile variant="cell"` children** (2026-09-25). Budget's totals compose it plain; a breakdown adds `title`, `divided` hairlines (serene-families.css draws each cell's left/top rule, the card's overflow clips the edge ones) and a `footer` (compose `MetaLine`). Freshdesk "By status" and Books "Aging" use `StatTile size="sm"` cells (lg mono value, `dot`, one-line legend label, `sub`). Never float a loose row of label: value pairs under the tiles. Server-safe. |
| `MetaLine`      | `MetaLine.tsx`      | `{ items, tone? }` | **THE quiet provenance line** — optional health dot (`live`/`warning`/`danger`) + short items joined by a middle dot, tertiary, wraps as whole items. Freshdesk sync health (StatStrip footer), Books "Read from Zoho". Server-safe. |
| `SplitWorkspace` | `SplitWorkspace.tsx` | `SplitWorkspace` / `SplitRail` / `SplitRailHeader` / `SplitRailList` / `SplitPane` | **THE two-card conversation layout** (2026-09-25): 340px rail card beside a flexible pane card (md+), one card at a time below md (the shell's useMediaQuery). Owns chrome + widths only; display stays with the caller's utilities (`hidden md:flex` on an empty pane; an unlayered class would beat Tailwind's layered ones). /sia, /whatsapp AND both loading.tsx compose it. Server-safe. |
| `EditDeleteActions` | `RowActions.tsx` | `{ onEdit onDelete deleting? subject }` | **THE Edit + Delete pair at the end of a record row** (ad creatives, training assets, notes): `control` Edit + `ghost-danger` Delete, both labelled (never `iconOnly` with a text child), `deleting` = Button's own pending state. The strong red stays in the caller's ConfirmDialog. |
| `Num`           | `Num.tsx`           | `{ children, style? }` | **THE inline numeric-token wrapper** (2026-07-06). Renders its children in `--font-mono` + `tabular-nums` with colour/weight **inherited** from the surrounding sentence (never re-tints). The number-font rule has two fonts — `--font-serif` for hero/stat values, `--font-mono` for secondary/technical — but a number embedded MID-SENTENCE (a tertiary caption, a drill-modal subtitle: `Aanya · 12 deals`) must keep the words in `--font-sans` while only the digits go mono. Compose `<Num>` for that case; **standalone hero/stat values set `--font-serif` directly, never this.** Display-only (A-06), server-component-safe. Used by the five count-bearing drill-modal subtitles + `AdSpendUploadModal` preview. |
...
**Cartesian frame rule:** Cartesian charts compose `CartesianChartFrame.tsx` — `<ChartFrame>` (paper container + `ResponsiveContainer`) and `cartesianDefaults(tokens)` (grid/axis/tooltip/legend prop objects, spread onto the Recharts elements) + `CARTESIAN_MARGIN`. Recharts resolves axes/tooltip by child *type*, so the elements stay in each chart's JSX — only the prop objects and the container are shared. Never re-inline those prop blocks; a new Cartesian chart spreads the defaults and adds its series renderer. (The unused `AreaChart`/`LineChart`/`PieChart`/`DonutChart`/`ButterflyChart` wrappers were deleted 2026-07-02 — live consumers import raw Recharts + this frame directly; `BarChart` is the surviving wrapper.)
...
**Rules:** never add a variant without a live consumer; never apply to a button whose icon swaps with a spinning mark mid-flight *and* whose meaning changes (a loading swap that keeps meaning, e.g. `MessageBar`, is fine — `serene-spin` overrides the hover transform).
...
Position: `fixed bottom-[--space-6] right-[--space-6]` desktop; mobile `bottom: calc(var(--elaya-fab-clearance) + var(--space-4) + safe-area-inset-bottom)` so a toast never covers a chat composer (mobile audit 2026-09-26).
...
| `ConversationRailRow` (ui/) | `ui/ConversationRailRow.tsx` | THE rail row, shared with /sia (2026-09-25; `ConversationRow.tsx` is deleted): avatar · name + time · phone line, hairline between rows, unread dot + semibold + accent time. Selected = the avatar ring and a semibold title only, never a wash.                                                            |
...
| `MessageBubble`          | `MessageBubble.tsx`          | Single message. Inbound (`--neu-surface-high`) / outbound (`--neu-chat-user-bg` — THE chat sender-bubble token, same contract as `ElayaMessageBubble`). Delivery icons (sent = single tertiary, delivered = double tertiary, read = double `--neu-info-deep`). Bot label. **Media:** `MediaPlaceholder` renders an inline `<img>` thumbnail (`maxWidth: 100%` of the bubble, the bubble carries the px cap, never the media; click→open) for images and `<video controls>` for videos (caption beneath); document/audio + any url-less media show the labelled chip + View link. A non-media message with blank content renders a muted serif-italic **"Unsupported message"** placeholder (never a blank bubble). |
...
| Component | File | Responsibility |
| --- | --- | --- |
| `ElayaChatShell` | `ElayaChatShell.tsx` | `'use client'` chat surface. Owns the page's two-column layout via the canonical `.serene-dossier-grid serene-dossier-grid--340` (audit F2 — never fork a grid class; chat in the 1fr column, identity sidebar right on lg+; **below lg there is no sidebar column** (mobile audit 2026-09-26: the flex-fill grid squeezed it into a 48px strip): the chat fills the screen, the role's starters render as a chip row above the composer while the transcript is empty, and Send feedback is an icon in the presence header via `useOptionalSuggestionFeedback`) plus transcript state, the SSE consumption loop against `POST /api/elaya/chat` (`meta`/`delta`/`tool`/`done`/`error` frames), composer — the shared `MessageBar` (`variant="default"`, ref-forwarded for starter-prompt focus; `sendOnEnter`: Enter sends and Shift+Enter is a newline on a fine pointer, a bare Enter is a newline on touch where the send knob sends; input restored on rejected send — never cleared on error; never hand-roll a composer here) with `leadingSlot={<DictationButton variant="composer" onError={toast.danger}/>}` for voice dictation (the same shared cluster the WhatsApp composer mounts — never re-inline the mic), cap banner, and the in-transcript status row (serif italic + small glyph — covers tool calls AND the first-token "Thinking…" wait). Presence header: breathing `LiaGlyph` in a 36px `--theme-accent-surface` tile; **no message counter** (the budget chip / "N of cap" mirror was removed 2026-06-12 — never reintroduce a visible count; at cap a right-aligned `--color-warning` "Daily limit reached" note appears and the composer swaps to the cap banner). A static glyph = Elaya absent — never pass `breathing={false}` while the shell is mounted. Cap/expiry are server-enforced; everything here is cosmetic state. |
...
| `ElayaStatusText` | `ElayaStatusText.tsx` | **THE morphing Elaya status line** (torph `TextMorph`, 2026-08-25). Tool-status phrases + "Thinking…" morph between each other instead of hard-swapping; inherits font/colour from the parent span. Spring = shared `SPRING_CONFIG` (V-13); torph gates on `prefers-reduced-motion` (`respectReducedMotion`). Mounted at the three status render points: `ElayaChatShell` header subtitle + in-transcript status row, mobile `ElayaChatScreen` `TypingBubble`. **Elaya is the ONLY surface allowed animated text** — never mount `TextMorph` anywhere else (greetings, page titles, empty states stay static) without a Decision Log entry. |
...
| `ElayaMemoryCard` (profile/) | `profile/ElayaMemoryCard.tsx` | **"What Elaya has learned about you"** (0237, 2026-09-25): the living memory of one person, one entry per line with its kind, remove on each (retired, never deleted), a kind + sentence box to add one. Mounted on `/profile` (own) and `/admin/users/[id]` (admin). Display + form state only; writes through `actions/elaya-memory.ts`. The settings twin is `settings/ElayaRequestsPanel.tsx` (the improvement-request queue on `/settings/elaya-requests`: question, her answer, the correction, her guess, a verdict + note she reads next). |
...
| -------------------------------- | --------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| `Loader2` inline spinners        | `Spinner` (since replaced by `LogoSpinner`)                                                 | CalledModal, PersonalDetailsCard, ProfileAvatarSection                                                                |
...
Every future modal **must** compose `src/components/ui/modal.tsx`. Every future loading indicator **must** use `LogoSpinner` (or an inline `SeedMandala` for in-control pending states). Every future form submit button **must** use `Button`. Search `src/components/ui/` before implementing any pattern inline.
