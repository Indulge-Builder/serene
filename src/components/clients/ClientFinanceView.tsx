// ClientFinanceView — the finance page body (server, display-only): the membership money
// as stat tiles and the money movements the twin has recorded. The live Zoho part
// (ClientZohoCards) streams in beneath when the client has a Zoho customer id.
...
export function ClientFinanceView({ detail }: { detail: ClientDetail }) {
      ...
      {!c.zoho_customer_id && (
        <div style={SHELL}>
          ...
          <div style={BODY}>
            <InfoRow
              label="Customer"
              value={<span style={{ color: 'var(--theme-text-tertiary)' }}>Not linked. Add the Zoho customer id on the <Link href={`${CLIENTS_PATH}/${c.id}`} style={{ color: 'var(--neu-accent-deep)' }}>client page</Link> and the invoices, payments and balances appear here.</span>}
            />
          </div>
        </div>
      )}
