...
// ClientHealthCard — the happiness score with its reasons, and a manual adjustment for a
...
import { adjustClientHealthAction } from '@/lib/actions/clients';
import { formatDate } from '@/lib/utils/dates';
import { HealthPill } from './HealthPill';
import type { ClientHealth } from '@/lib/types/client';

export function ClientHealthCard({ clientId, health }: { clientId: string; health: ClientHealth }) {
  ...
  function submit() {
    start(async () => {
      const res = await adjustClientHealthAction({ client_id: clientId, delta: Number(delta), note });
