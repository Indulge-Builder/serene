...
// ClientPeopleCard — the humans under a membership (a couple is one client with two names;
...
import { addClientPersonAction, deleteClientPersonAction } from '@/lib/actions/clients';
import type { ClientPersonRow } from '@/lib/types/client';

const RELATIONS = ['primary', 'spouse', 'partner', 'child', 'parent', 'sibling', 'staff', 'other'] as const;

export function ClientPeopleCard({ clientId, people }: { clientId: string; people: ClientPersonRow[] }) {
  ...
  function submit() {
    start(async () => {
      const res = await addClientPersonAction({ client_id: clientId, name: form.name, relation: form.relation, phone_e164: form.phone || null, note: form.note || null, can_request: true });
  ...
  function remove(id: string) {
    start(async () => {
      const res = await deleteClientPersonAction({ client_id: clientId, person_id: id });
      if (res.error) { toast.danger(res.error); return; }
      router.refresh();
    });
  }
