...
export function ClientAppCard({ detail }: { detail: ClientDetail }) {
  const appEvents = detail.events.filter((e) => e.kind.startsWith('app_') || e.kind === 'location');
  const linked = Boolean(detail.client.app_member_id);
  return (
    <div style={SHELL}>
...
export function ClientMoneyCard({ detail }: { detail: ClientDetail }) {
        ...
        <InfoRow
          ...
          value={c.zoho_customer_id ? (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--space-2)' }}>
              <Link href={clientFinancePath(c.id)} style={{ color: 'var(--neu-accent-deep)' }}>See finance</Link>
...
export function ClientRelationsCard({ detail }: { detail: ClientDetail }) {
...
export function ClientAnticipationsCard({ detail }: { detail: ClientDetail }) {
...
export function ClientNarrativeCard({ detail }: { detail: ClientDetail }) {
