...
// ClientWhatsAppCard — the client's WhatsApp group from the Sia archive: which group, how
...
import { linkClientGroupAction, unlinkClientGroupAction } from '@/lib/actions/clients';
import { formatCount } from '@/lib/utils/numbers';
import { formatRelativeTime } from '@/lib/utils/dates';
import type { ClientGroupSummary } from '@/lib/types/client';
import type { SiaGroupRow } from '@/lib/services/sia-service';

export function ClientWhatsAppCard({ clientId, group, canLink }: { clientId: string; group: ClientGroupSummary | null; canLink: boolean }) {
  ...
  function link(jid: string) {
    start(async () => {
      const res = await linkClientGroupAction({ client_id: clientId, group_jid: jid });
  ...
  function unlink() {
    if (!group) return;
    start(async () => {
      const res = await unlinkClientGroupAction({ client_id: clientId, group_jid: group.group_jid });
      if (res.error) { toast.danger(res.error); return; }
    ...
    <div style={{ background: 'var(--theme-paper)', border: '1px solid var(--theme-paper-border)', borderRadius: 'var(--neu-radius-card)', boxShadow: 'var(--shadow-1)', overflow: 'hidden' }}>
      ...
      <div style={{ padding: 'var(--space-4) var(--space-6) var(--space-5)', display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
        {group ? (
          ...
          <EmptyState variant="inline" title="No group linked." description={canLink ? 'Pick the client\'s group so their conversation joins this profile.' : 'A bishop or admin links the group from Sia.'} />
        )}
        {picking && !group && (
