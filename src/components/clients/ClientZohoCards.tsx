// ClientZohoCards — the live Zoho part of the client finance page (server, display-only):
...
export function ClientZohoCards({ clientId, zohoCustomerId, orgId, finance, failure }: { clientId: string; zohoCustomerId: string; orgId: string; finance: ClientFinance | null; failure: string | null }) {
  if (!finance) {
    return (
      <div style={SHELL}>
        ...
        <div style={BODY}>
          ...
          {failure && <div><RefreshClientFinanceButton clientId={clientId} zohoCustomerId={zohoCustomerId} /></div>}
        </div>
      </div>
    );
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      ...
      <div style={SHELL}>
        <CardHeader
          icon={Landmark}
          label="Zoho customer"
          right={<span style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 'var(--space-3)', fontSize: 'var(--text-xs)', color: 'var(--neu-header-ink)' }}>read {formatRelativeTime(finance.fetchedAt)}<RefreshClientFinanceButton clientId={clientId} zohoCustomerId={zohoCustomerId} /></span>}
        />
        <div style={BODY}>
