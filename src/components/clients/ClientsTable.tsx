...
// ClientsTable — the /clients dense table. Display-only (A-06): renders the rows the server
...
import { CLIENT_TIERS } from '@/lib/constants/client-facets';
...
export function ClientsTable({ clients, hasFilters }: { clients: ClientListItem[]; hasFilters: boolean }) {
  if (clients.length === 0) {
    return (
      <div style={{ background: 'var(--theme-paper)', border: '1px solid var(--theme-paper-border)', borderRadius: 'var(--neu-radius-card)', boxShadow: 'var(--shadow-1)', padding: 'var(--space-12) var(--space-6)' }}>
        <EmptyState
          icon={Users}
          title={hasFilters ? 'Nobody matches these filters.' : 'No clients yet.'}
          description={hasFilters ? 'Try clearing a filter.' : 'Add the first client with the button above.'}
        />
      </div>
    );
  }
    ...
    <div style={{ background: 'var(--theme-paper)', border: '1px solid var(--theme-paper-border)', borderRadius: 'var(--neu-radius-card)', boxShadow: 'var(--shadow-1)', overflow: 'hidden' }}>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th className="label-micro" style={HEAD}>Client</th>
...
const ClientRow = memo(function ClientRow({ c }: { c: ClientListItem }) {
