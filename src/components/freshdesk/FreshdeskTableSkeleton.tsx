// The /freshdesk table skeleton — shared scaffold blocks, table-shaped. Server-safe.

import { Shimmer, skeletonStagger } from '@/components/ui/PageSkeletons';

export function FreshdeskTableSkeleton({ rows = 10 }: { rows?: number }) {
  return (
    <div
      style={{
    ...
    >
      ...
      >
        {[24, 160, 80, 70, 60, 80, 90, 70, 60].map((w, i) => (
      ...
      {Array.from({ length: rows }).map((_, i) => (
        ...
        >
          <Shimmer w={36} h={12} delay={skeletonStagger(i)} />
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
            <Shimmer w="55%" h={12} delay={skeletonStagger(i)} />
