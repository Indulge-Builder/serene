...
// MemberObservationCard — THE one box for what the team knows about a member. A person
...
import { addMemberObservationAction } from '@/lib/actions/members';
import { formatDate } from '@/lib/utils/dates';
import { CLIENT_FACETS, FACT_KEY_LABELS, type MemberFacet } from '@/lib/constants/member-facets';
import type { MemberFactView, MemberObservationResult } from '@/lib/types/member';
...
export function MemberObservationCard({ clientId, notes }: { clientId: string; notes: MemberFactView[] }) {
  const router = useRouter();
  const [draft, setDraft] = useState('');
  const [error, setError] = useState<string | null>(null);
  const [last, setLast] = useState<MemberObservationResult | null>(null);
  ...
  function submit() {
    ...
    setError(null);
    start(async () => {
      const res = await addMemberObservationAction({ member_id: clientId, text });
