...
| `AgentIdentity.tsx` | **THE one agent identity on /performance** (mobile audit 2026-09-26): `AgentIdentityHeader` (avatar `lg` + live pip, name, domain pill, lead count; `showName={false}` when a title bar already names the agent) and `AgentStatRow` (Calls · Leads · Won · Revenue as four `StatAtom` tap targets; a value not loaded yet is an em dash, never a word). `AgentDetailPanel` AND `FounderDrillDownDeck` compose both, so a founder on a phone sees the same agent whichever domain filter is on; the deck's private `DeckTile` is deleted. The agent's own scorecard keeps the ui/StatTile Today strip (today's pulse, a different surface) |
...
| `StatAtom.tsx` | Single pastel stat tile (`AgentDetailPanel` stats row + `DomainOverviewPanel` health cards). Optional `onClick` → pressable `motion.button` (`.serene-pressable` press-scale + cursor + focus ring; the deck composes the same tiles through `AgentStatRow` since 2026-09-26); absent → original static `motion.div`. **BOTH mount sites now pass `onClick`** — `AgentDetailPanel`'s four tiles open the agent drill modals; `DomainOverviewPanel`'s four tiles open `DomainLeadsDrillModal` (2026-06-24). A static (no-`onClick`) consumer still renders the byte-identical `motion.div`. |
...
| `DomainOverviewPanel.tsx` | Founder Domains tab — 4 stats per domain (incl. Deals Closed) + month-pinned `DomainTargetMeter` + founder/admin inline target edit (`upsertDomainTargetAction`); mobile = CSS scroll-snap carousel (no library). The comparative BarChart's Leads/Calls/Revenue toggle is the shared `TabSelector` (variant `accent`, `indicatorLayoutId="domain-metric-toggle"` — distinct from the founder shell's `founder-perf-tabs` pill, which is co-mounted). **Globally scoped (2026-06-25):** the `scopeDomain` prop (from `resolveDomainParam` at the page) collapses the tab to ONE card + a single-bar chart via `visibleDomains`; null = all 4. The period/date refetch passes `scopeDomain` to `getDomainHealthMetricsAction`. **The 4 stat tiles are tap targets:** each passes `StatAtom onClick` routed through a `DomainTileTarget` `{ surface:'leads'\|'deals' }` union — **Leads + Calls → `DomainLeadsDrillModal`** (`all`/`calls`); **Deals Closed + Revenue → `DomainDealsDrillModal`** (the card counts the `deals` table by `won_at`, so a deal list — not a lead list — ties out, 2026-06-25). ONE of each modal at the panel level, drill state in the panel, mirrors `AgentDetailPanel` |
| `DomainLeadsDrillModal.tsx` | The leads behind a clicked DOMAIN-card **Leads or Calls** tile (Domains tab, 2026-06-24) — a thin caller of `LeadDrillModal` supplying `getDomainLeadsDrillAction`. `kind: 'all' \| 'calls'` maps the tile to the slice; domain-scoped (no agent). (`'won'` exists in the action/type but the Deals tiles route to `DomainDealsDrillModal` now — the `won` lead path is unreachable from the panel.) Rows link to the dossier — the domain twin of `AgentLeadsPredicateDrillModal`. No fetch lifecycle/row/chrome of its own. **Per-kind `renderMeta` (the row's calm secondary line):** `all` → assignee chip (`xs` `Avatar` initials + name, "Unassigned" italic-tertiary when null); `calls` → last call outcome (quiet `PhoneCall` glyph + `CALL_OUTCOME_LABELS`). Token-only, no accent pill — the status pill stays the row's single coloured element |
| `DomainDealsDrillModal.tsx` | The DEALS behind a clicked DOMAIN-card **Deals Closed or Revenue** tile (Domains tab, 2026-06-25) — a thin caller composing `DrillModalShell` + the shared `DealDrillRow`, supplying `getDomainDealsDrillAction` (reuses `getDealsByRole`, `won_at`-in-range, domain-scoped). Lists DEALS, not leads, so the drill total equals the card's deals/revenue number. Fetch-on-open, flat + bounded (pageSize 200 in the action), no load-more — matching the other Domains-tab drills. A SEPARATE flat stack from `LeadDrillModal` (a deal is a different row shape than `LeadListItemWithAssignee`); the two share only `DrillModalShell` |
...
| `AgentDealsDrillModal.tsx` | Agent's won deals — fetch-on-open, page load-more via `getAgentDealsScopedAction` (existing `getDealsByRole` path, `filters.agent_id`). Rows render through the shared `DealDrillRow` (R-01, 2026-06-25 — same row the domain deals drill uses) |
| `DealDrillRow.tsx` | THE single deal row (contact · type · won date · amount) for the performance deal drills. Extracted from `AgentDealsDrillModal` (2026-06-25) so the Domains-card Deals/Revenue drill renders an identical row — never copy-paste a second deal drill row (R-01; the `LeadDrillRow` precedent). Links to the lead dossier (`?from=/performance`) when the deal has a lead; walk-ins (no `lead_id`) stay a plain row. `.serene-pressable` hover/press. Used by `AgentDealsDrillModal` + `DomainDealsDrillModal` |
