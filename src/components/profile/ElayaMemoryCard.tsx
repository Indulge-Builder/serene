  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      ...
      <div style={{ display: 'flex', gap: 'var(--space-2)', alignItems: 'center', flexWrap: 'wrap' }}>
        <FormSelect aria-label="Kind" fullWidth={false} value={kind} onValueChange={(nextValue) => setKind(nextValue as ElayaMemoryKind)} style={{ width: 'auto' }}>
          {ELAYA_MEMORY_KIND_OPTIONS.map((o) => <option key={o.id} value={o.id}>{o.label}</option>)}
        </FormSelect>
        <Input
          style={{ width: '100%', flex: 1, minWidth: 200 }}
          value={statement}
