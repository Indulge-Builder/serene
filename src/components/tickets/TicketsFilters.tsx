...
export function TicketsFilters({ queendoms, staff }: { queendoms: QueendomSummary[]; staff: StaffOption[] }) {
  ...
  const activeCount = [params.get('search'), statuses.length ? 'x' : null, queendom, assignee, category, mine ? 'x' : null].filter(Boolean).length;
  ...
  return (
    ...
    >
      <FilterDropdown label="Status" items={TICKET_STATUSES.options} selected={statuses} onChange={setStatuses} multi menuPortal />
      {queendoms.length > 1 && (
