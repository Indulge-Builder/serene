...
interface TimeState { hour: number; minute: number; meridiem: Meridiem; }
...
function WheelColumn({ values, selected, onSelect, format, ariaLabel }: WheelColumnProps) {
  ...
  const scrollToIndex = useCallback((index: number, smooth: boolean) => {
    ...
    setScrollTop(clamped * ITEM_HEIGHT);
    window.setTimeout(() => { syncingRef.current = false; }, smooth ? 280 : 0);
  }, [values.length]);
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          left:           0,
          right:          0,
          height:         WHEEL_PADDING + ITEM_HEIGHT * 0.5,
          background:     'linear-gradient(to bottom, var(--theme-paper) 15%, transparent)',
          pointerEvents:  'none',
          zIndex:         2,
        }}
      />
      ...
      <div
        ...
        style={{
          ...
          right:          0,
          height:         WHEEL_PADDING + ITEM_HEIGHT * 0.5,
          background:     'linear-gradient(to top, var(--theme-paper) 15%, transparent)',
          pointerEvents:  'none',
          zIndex:         2,
        }}
      />
      ...
      >
        <div style={{ paddingTop: WHEEL_PADDING, paddingBottom: WHEEL_PADDING }}>
          {values.map((v, i) => {
            const itemCenter = WHEEL_PADDING + i * ITEM_HEIGHT + ITEM_HEIGHT / 2;
            const dist = Math.abs(centerY - itemCenter) / ITEM_HEIGHT;
            ...
            return (
              <button
                ...
                style={{
                  ...
                  fontWeight:     isCentered ? 'var(--weight-semibold)' : 'var(--weight-medium)',
                  fontVariantNumeric: 'tabular-nums',
                  color:          isCentered ? 'var(--theme-accent)' : 'var(--theme-text-secondary)',
                  cursor:         'pointer',
                  opacity,
...
export function TimePicker({
  ...
  const updatePanelPosition = useCallback(() => {
    ...
    if (!rect) return;
    const flipLeft = rect.left + PANEL_WIDTH > window.innerWidth - 8;
    const spaceBelow = window.innerHeight - rect.bottom;
    const flipUp = spaceBelow < PANEL_EST_HEIGHT && rect.top > spaceBelow;
    const left = flipLeft ? rect.right - PANEL_WIDTH : rect.left;
  ...
  const panelState = draft ?? (normalisedValue ? parse(normalisedValue) : DEFAULT_STATE);
    ...
    >
      <button
        ...
        style={{
          ...
          color:        normalisedValue ? 'var(--theme-text-primary)' : 'var(--theme-text-tertiary)',
      ...
      >
        <Clock
          aria-hidden="true"
          style={{ width: 13, height: 13, strokeWidth: 1.5, color: 'var(--theme-text-tertiary)', flexShrink: 0 }}
        />
