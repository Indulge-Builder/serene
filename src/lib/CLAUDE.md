...
| `actions/leads.ts` | Lead lifecycle — `addLeadCallNote`, `addLeadNote`, `updateLeadStatus`, `assignLead`, `createManualLead`, `createLeadTaskAction`, `recordDeal`, `updateLeadEmail`, `updateLeadDomain`, `updateLeadSource` (renamed from `updateLeadUtmSource` in the 0065 attribution refactor), `updateLeadCity`, `updateLeadInterests` (Phase 1.1b — same `assertLeadFieldEditAccess` gate; drops unknowns vs lead domain via `extractServiceInterests`; activity `lead_interests_updated {old, new}`; no-op edits skip write+activity), `updatePersonalDetails`, `searchLeadsAction`, `exportLeadsAction`. **`updateScratchpad` removed** — scratchpad dropped in migration 0061. |
