...
| `services/revival-service.ts` | **SERVER ONLY.** THE `revival_candidates` + `revival_policies` access (Lead Revival R1, admin client — mirrors elaya-actions-service; interim `as any` cast on `.from` until 0119 + `database.ts` regen). Config: `getActiveRevivalPolicies()` (per sweep run, never module-cached — sla_policies pattern), `getAllRevivalPolicies()` (settings), `updateRevivalPolicy(triggerStatus, patch)` (admin client; the gated action is the trust boundary). Silence finder: `findSilentLeadsForStatus(status, silenceDays)` — leads in `status` silent past the threshold with NO open candidate (the anti-join — the partial UNIQUE index is the DB backstop); excludes archived + unassigned; bounded per run. Cap: `countAutoRevivesToday(agentId)` — native `.eq('assigned_to')` filter (NOT a leads embed — head:true drops it), fails CLOSED to Infinity. Ledger: `insertRevivalCandidate` (open/actioned; 23505 unique-violation swallowed = the one-open race guard), `markCandidateResolved(id, status, by)` (resolve-once `open →` flip, the A-11 carve-out), `getOpenCandidateForLead` (dossier), `getOpenCandidatesForCaller(sessionClient)` (the review-predicate read — RLS-scoped on the SESSION client). **Revival NEVER writes the leads row.** |
| `services/revival-gate.ts` | **SERVER ONLY.** `judgeLeadForRevival({leadId, triggerStatus})` — THE note-AI suppression gate (Lead Revival R1). A SINGLE-SHOT structured judgment reusing the Elaya provider/PII layer wholesale (R-01 / D-01): `getLeadNotesFull` → `maskPii` → `resolveLlmForJob('routing')` (Haiku) → `adapter.complete()` with **NO tools** → JSON parse. NO second LLM integration, no `@anthropic-ai/sdk` import. The suppression bias lives in the system prompt (own-network / "only wanted details" / affordability-dead / pure-NR → `unsure`). **Fails CLOSED to `unsure`** on empty notes / malformed / thrown — only an exact `"revive"` string can become a task. |
