...
| `src/trigger/usage-rollup.ts` | TWO scheduled tasks, both writing the SAME `usage_daily` via the SAME idempotent `rollupUsageForDays` core (Option 2). `rollupUsageTodayTask` — re-rolls **today (IST)** every 15 min (`*/15 * * * *`, `Asia/Kolkata`). `rollupUsageNightlyTask` — 00:20 IST (`20 0 * * *`, `Asia/Kolkata`): finalises the **prior IST day** (+ today as cheap insurance), then `pruneOldHeartbeats(30)` drops raw ticks > 30 days (`usage_daily` is never pruned). Recompute-and-UPSERT (overwrite, never increment) → re-running yields identical rows. IST dates via `istDateString` (usage-service) — never re-fork IST math. |
