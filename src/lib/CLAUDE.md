...
| `constants/themes.ts` | THE theme vocabulary (`THEME_KEYS`/`THEME_OPTIONS`/`ThemeKey`/`DEFAULT_THEME`/`isThemeKey()` via `defineEnum`) + the SSR theme mirror: `THEME_COOKIE` (`eia-theme`, read by the root layout to stamp `data-theme` on `<html>` server-side — the zero-flash mechanism) and `persistThemeCookie()` (client-only writer — ThemeInitializer on load, ThemeSelector on switch). `profiles.theme` stays the source of truth; never re-inline the five theme keys. |
