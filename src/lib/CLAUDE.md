...
| `services/whatsapp-api.ts` | **SERVER ONLY.** Gupshup/Meta Cloud API HTTP client. Exports: `sendTextMessage`, `sendTemplateMessage`, `sendMediaMessage`, `uploadMedia`, `getMediaDownloadUrl`, `verifyMetaSignature` (HMAC-SHA256 + `timingSafeEqual`), `sendLeadAssignmentNotification`, `sendFounderLeadNotification`, `WEBHOOK_VERIFY_TOKEN`, `BUSINESS_ACCOUNT_ID`. Internal `logNotification()` writes every template attempt to `whatsapp_notification_logs` (stores last-4 phone digits only, never full numbers). Never import in client components. |
