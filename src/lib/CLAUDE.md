...
| `services/whatsapp-api.ts` | **SERVER ONLY.** Meta Cloud API HTTP client — no DB, no Supabase. Exports: `sendTextMessage`, `sendTemplateMessage`, `sendMediaMessage`, `uploadMedia`, `getMediaDownloadUrl`, `verifyMetaSignature` (HMAC-SHA256 + `timingSafeEqual`), `WEBHOOK_VERIFY_TOKEN`, `BUSINESS_ACCOUNT_ID`. Never import in client components. |
