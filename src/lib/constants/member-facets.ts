// constants/member-facets.ts — THE vocabulary of the member twin (migration 0194,
// member-ticket-plan.md section 5). Facets, sources, polarity, event kinds, relation kinds,
// anticipation kinds, membership tiers. The SQL CHECKs on member_facts.facet / .source /
// .polarity and on member_anticipations.kind mirror these lists exactly — a new value is one
...
export type MemberFacet = (typeof CLIENT_FACETS.values)[number];

/** Which facets the Essentials card shows, in order; the rest go to Preferences. */
export const ESSENTIAL_FACETS: readonly MemberFacet[] = ["address", "family", "dietary", "contact_rule"];
export const PREFERENCE_FACETS: readonly MemberFacet[] = ["preference", "interest", "travel", "occasion", "budget_signal"];
...
/** members.tier — from the membership types the sheets and Zoho use. */
...
export type MemberTier = (typeof CLIENT_TIERS.values)[number];

/** "Monthly Trial" → monthly_trial; unknown labels → null (never invent a tier). */
export function tierFromLabel(label: string | null | undefined): MemberTier | null {
  const key = (label ?? "").trim().toLowerCase().replace(/\s+/g, "_");
  return (CLIENT_TIERS.values as readonly string[]).includes(key) ? (key as MemberTier) : null;
}
