...
| `search_member_history` | read (bridged) | all staff | `elayaData.searchMemberHistoryFor` → `searchSiaMessages` (the `simple` FTS index) scoped to the member's group, newest first, capped 30. Empty → "nothing on record about that", never a guess |
