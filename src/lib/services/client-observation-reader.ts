// client-observation-reader.ts — read one free-form observation about a client the way a
...
import { CLIENT_FACETS, FACT_KEY_LABELS, FACT_POLARITIES, RELATION_KINDS, type ClientFacet, type FactPolarity } from "@/lib/constants/client-facets";

const LOG = "[client-observation-reader]";
export const OBSERVATION_PROMPT_VERSION = "observation-v2";

export type ObservedFact = { facet: Exclude<ClientFacet, "note">; key: string; value: string; polarity: FactPolarity };
...
const SYSTEM_PROMPT = `You file a concierge team member's observation about a wealthy client into the client's profile.
...
- Only what the observation actually says about THIS client. Never invent. Never infer a fact from a guess.
- A detail about a family member or staff (their diet, their birthday, their taste) goes INSIDE that person's family fact value ("Priya, vegetarian"), never into the client's own dietary / preference / identity drawers.
 ...
 */
export async function readClientObservation(original: string): Promise<ObservationReading | null> {
