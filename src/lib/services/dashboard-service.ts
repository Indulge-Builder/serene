...
export const getDashboardSummary = cache(
  async (
    ...
    const { data, error } = await supabase.rpc('get_dashboard_summary', {
      ...
      p_initial_domain: (initialDomain ?? null) as AppDomain | undefined,
      p_date_from:      (dateRange?.from ?? null) as string | undefined,
      p_date_to:        (dateRange?.to   ?? null) as string | undefined,
    });
...
export async function getAgentTasksSummary(agentId: string): Promise<import('@/lib/types').DashboardAgentTask[]> {
  ...
  return withRedisCache(cacheKey, REDIS_TTL.DASHBOARD_AGENT_TASKS, async () => {
...
export async function getAgentRecentActivity(
  ...
  const supabase = createAdminClient();
  const { data, error } = await supabase.rpc('get_recent_lead_activity', {
    p_role:    role ?? 'agent',
    // rpcDomain may legitimately be null (all-org) — the SQL param is nullable but
    // the generated arg type is the bare enum, hence the assertion.
    p_domain:  rpcDomain as AppDomain,
    p_user_id: agentId,
    p_scope:   scope,
  });
  if (error) throw error;
...
function normalizeLeadStatusSummary(raw: LeadStatusSummary): LeadStatusSummary {
...
export async function getLeadStatusSummary(
  ...
  // Key mirrors the RPC scope exactly (Q-16) — role + domain + range.
  const cacheKey = REDIS_KEYS.dashboardLeadStatus(rpcRole, rpcDomain as string, dateRange?.from, dateRange?.to);

  return withRedisCache(
    cacheKey,
    REDIS_TTL.DASHBOARD_LEAD_STATUS,
    async () => {
      ...
      // Explicit nulls (not undefined) are intentional — PostgREST sends SQL NULL;
      // the generated optional-arg types only admit undefined, hence the assertions.
      const { data, error } = await supabase.rpc('get_lead_pipeline_refresh', {
        p_role:      rpcRole,
        p_domain:    rpcDomain,
        p_date_from: (dateRange?.from ?? null) as string | undefined,
        p_date_to:   (dateRange?.to   ?? null) as string | undefined,
      ...
      return normalizeLeadStatusSummary({
        totals:  rpcData.totals  ?? [],
        byAgent: rpcData.byAgent ?? [],
      });
    },
    // Cached hits are re-normalized so stale envelopes never leak raw jsonb shapes.
    normalizeLeadStatusSummary,
  );
}
...
// IST-pinned via formatDate (2026-07-02): this runs on the SERVER, where
// toLocale* rendered in the runtime TZ (UTC on Vercel) — chart labels were
// UTC days/hours, so a 2 AM IST lead landed under the previous day's label.
function formatBucketLabel(date: Date, bucketMs: number): string {
  if (bucketMs <= 3_600_000) {
    // Hourly — show H:MM AM/PM
...
export async function getLeadVolumeByRange(
  ...
  return withRedisCache(cacheKey, REDIS_TTL.DASHBOARD_LEAD_VOLUME, async (): Promise<LeadVolumeSummary> => {
    ...
    return buildVolumeSeries(rows, fromMs, toMs, bucketMs);
  });
}
...
export async function getLeadVolumeByDomains(
  ...
  return withRedisCache(cacheKey, REDIS_TTL.DASHBOARD_LEAD_VOLUME, async (): Promise<MultiDomainVolumeSummary> => {
    const from     = new Date(dateRange.from);
    const to       = new Date(dateRange.to);
    const fromMs   = from.getTime();
    const toMs     = to.getTime();
...
export async function getLeadsByCampaign(
  ...
  const rpcDomain = (role === 'manager' ? domain : targetDomain ?? domain) as AppDomain;
  // Key mirrors the RPC scope exactly (Q-16) — role + domain + range.
  const cacheKey = REDIS_KEYS.dashboardCampaigns(rpcRole, rpcDomain as string, dateRange?.from, dateRange?.to);

  return withRedisCache(cacheKey, REDIS_TTL.DASHBOARD_CAMPAIGNS, async () => {
    // EXECUTE revoked from `authenticated` (0102, F-1) — admin client; scope args
    // are session-derived by the dashboard action/page (Q-13).
    const supabase = createAdminClient();
    // Explicit nulls (not undefined) are intentional — PostgREST sends SQL NULL;
    // the generated optional-arg types only admit undefined, hence the assertions.
    const { data, error } = await supabase.rpc('get_campaign_pipeline_refresh', {
      p_role:      rpcRole,
      p_domain:    rpcDomain,
      p_date_from: (dateRange?.from ?? null) as string | undefined,
      p_date_to:   (dateRange?.to   ?? null) as string | undefined,
    ...
    return (data ?? []) as CampaignStatusMix[];
  });
}
