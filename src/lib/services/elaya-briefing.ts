...
export async function gatherBriefingData(slot: BriefingSlot, now = new Date()): Promise<BriefingData> {
  ...
  const [fdByQ, fdNotable, fdCreated, groups, waits, lines, toneCounts, toneEvents, feedback, occasions, renewals, staffMsgs, fdAgents, siaTix, waiting, memberQueendoms] = await Promise.all([
    q(`${WIN} select q.name as queendom, count(*) filter (where t.created_at >= w.f and t.created_at < w.t) as new_in_window, count(*) filter (where t.due_by >= w.f and t.due_by < w.t and (t.resolved_at is null or t.resolved_at > t.due_by)) as went_overdue_in_window from w cross join freshdesk_tickets t join queendoms q on q.freshdesk_group_id = t.group_id where t.created_at >= w.f or t.due_by >= w.f group by 1 order by 2 desc`, 20),
  ...
  const by_queendom: Q[] = [...names].sort().map((name) => {
    ...
    return {
      queendom: name,
      tickets: fd ? { new_in_window: n(fd.new_in_window), went_overdue_in_window: n(fd.went_overdue_in_window) } : { new_in_window: 0, went_overdue_in_window: 0 },
...
export function plainBriefing(d: BriefingData, slot: BriefingSlot, firstName: string): string {
  ...
  if (d.freshdesk) {
    const tot = d.freshdesk.by_queendom.reduce<{ c: number; o: number }>((a, r) => ({ c: a.c + n(r.new_in_window), o: a.o + n(r.went_overdue_in_window) }), { c: 0, o: 0 });
    lines.push(`*Requests:* ${tot.c} new Freshdesk tickets, ${tot.o} went overdue. ${d.freshdesk.by_queendom.slice(0, 4).map((r) => `${r.queendom} ${n(r.new_in_window)} new / ${n(r.went_overdue_in_window)} overdue`).join(', ')}.`);
  }
...
**By queendom** — one line per queendom (Ananyshree, Anishqa, Sanika), in this shape: "Ananyshree: 30 new tickets, 12 went overdue; <the one thing its queen should fix today, or 'steady'>". Both numbers are the window's (new = created in the window; went overdue = passed their due time during the window while still unresolved). No backlog, no "open right now", no all-time number anywhere in the brief. Add a second line only when that queendom has a member waiting or unhappy. Nothing else.
