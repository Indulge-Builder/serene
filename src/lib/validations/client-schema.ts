...
import { CLIENT_FACETS, CLIENT_TIERS, CLIENT_STATUSES, FACT_POLARITIES } from "@/lib/constants/client-facets";

// ─────────────────────────────────────────────
// Clients — Zod schemas (migration 0194). Every action in actions/clients.ts
...
export const ClientIdSchema = z.object({ client_id: uuidField(formErrors.generic) });
...
/** The Observation box: one free-form sentence or paragraph about the client. */
export const AddClientObservationSchema = z.object({
  client_id: uuidField(formErrors.generic),
  text: shortText(2000).pipe(z.string().min(3, formErrors.required)),
});

export const AddClientPersonSchema = z.object({
  client_id: uuidField(formErrors.generic),
...
export const UpdateClientPersonSchema = AddClientPersonSchema.partial().extend({
  person_id: uuidField(formErrors.generic),
  client_id: uuidField(formErrors.generic),
});

export const DeleteClientPersonSchema = z.object({
  person_id: uuidField(formErrors.generic),
  client_id: uuidField(formErrors.generic),
});

export const LinkClientGroupSchema = z.object({
  client_id: uuidField(formErrors.generic),
  group_jid: z.string().trim().regex(/@g\.us$/, formErrors.generic),
});

export const UnlinkClientGroupSchema = z.object({
  client_id: uuidField(formErrors.generic),
  group_jid: z.string().trim().regex(/@g\.us$/, formErrors.generic),
});
...
export type AddClientObservationInput = z.infer<typeof AddClientObservationSchema>;
export type AddClientPersonInput = z.infer<typeof AddClientPersonSchema>;
export type UpdateClientPersonInput = z.infer<typeof UpdateClientPersonSchema>;
