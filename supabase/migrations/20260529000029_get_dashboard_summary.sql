-- get_dashboard_summary(p_role, p_domain, p_user_id)
-- Single RPC replacing 4 individual dashboard queries.
-- Returns jsonb with keys: agent_tasks, agent_activity, lead_status, campaigns.
-- Role-based filtering mirrors the exact logic in dashboard-service.ts service functions.
-- SECURITY DEFINER: RLS does not fire. Access control is enforced explicitly via p_role/p_user_id.
...
BEGIN
  ...
  --    Tasks: assigned_to = p_user_id, status = 'to_do', due today or overdue or null
  --    New leads: assigned_to = p_user_id, status = 'new', not archived
  -- ─────────────────────────────────────────────────────────────────
  ...
  --    Last 10 lead_activities where actor_id = p_user_id
  -- ─────────────────────────────────────────────────────────────────
  ...
  --    manager: domain-scoped. admin/founder: all.
  --    Returns: totals (per status) + byAgent (per agent breakdown)
  -- ─────────────────────────────────────────────────────────────────
  WITH lead_rows AS (
    SELECT
      l.status,
  ...
  --    manager: domain-scoped. admin/founder: all.
  --    Only leads with utm_campaign set.
  -- ─────────────────────────────────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION get_dashboard_summary(text, text, uuid) TO authenticated;
