-- Replace the agent_tasks section of get_dashboard_summary.
-- Old: only gia_followup tasks (JOIN task_gia_meta) due today or overdue.
-- New: all 3 categories (personal, group_subtask, gia_followup) with active
--      statuses (to_do, in_progress, in_review), sorted by:
--        1. overdue first
--        2. priority order (urgent=1, high=2, normal=3)
--        3. due_at ASC NULLS LAST
-- Context label: lead name for gia_followup, group title for group_subtask,
--                null for personal (title is sufficient).
...
BEGIN
  ...
  WITH task_rows AS (
    SELECT
      ...
      -- link target: lead dossier for gia_followup, /tasks for others
      CASE
