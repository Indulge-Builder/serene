-- Migration 0069: Global dashboard date filter
--
-- Adds nullable p_date_from / p_date_to to three RPCs so the dashboard's
-- global date filter can scope pipeline and campaign data to a specific window.
--
-- Critical Date-Field Rule: filtering is on leads.created_at (intake/cohort).
-- NOT on status_changed_at — the filter answers "leads that came in during X".
--
-- Changes:
--   1. get_dashboard_summary       — new params p_date_from/p_date_to applied
--                                    to lead_status + campaigns CTEs only.
--                                    agent_tasks + agent_activity are unaffected.
--   2. get_lead_pipeline_refresh   — new params; applied to lead WHERE clause.
--   3. get_campaign_pipeline_refresh — new params; applied to lead WHERE clause.
--
-- All three: when p_date_from IS NULL the filter is skipped (all-time behaviour,
-- backwards compatible with all existing callers that omit the params).

-- ─────────────────────────────────────────────────────────────────
-- 1. get_dashboard_summary (5-param overload)
--    Drop the 4-param overload first — we replace it, not overload it.
-- ─────────────────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS get_dashboard_summary(text, app_domain, uuid, app_domain);
...
BEGIN
  ...
  -- ─────────────────────────────────────────────────────────────────
  -- Role branch: agents skip pipeline/campaign CTEs entirely.
  -- ─────────────────────────────────────────────────────────────────
  ...
  WITH lead_rows AS (
    ...
    WHERE l.archived_at IS NULL
      ...
      -- Date filter on created_at (intake/cohort date)
...
-- ─────────────────────────────────────────────────────────────────
-- 2. get_lead_pipeline_refresh — add date params
-- ─────────────────────────────────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION get_lead_pipeline_refresh(text, app_domain, timestamptz, timestamptz) TO authenticated;

-- ─────────────────────────────────────────────────────────────────
-- 3. get_campaign_pipeline_refresh — add date params
-- ─────────────────────────────────────────────────────────────────
...
GRANT EXECUTE ON FUNCTION get_campaign_pipeline_refresh(text, app_domain, timestamptz, timestamptz) TO authenticated;
