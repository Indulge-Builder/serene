-- Migration 0070: Fix per-agent / per-campaign totals in dashboard pipeline RPCs
--
-- agent_counts and campaign_agg used COUNT(*) on the per-status subquery rows,
-- which counts status buckets (1–7) instead of leads. Stacked bars divided
-- segment counts by that wrong total, producing widths >100% and broken visuals.
-- Fix: SUM(cnt) for lead/campaign totals. Sort order (ORDER BY total DESC) is
-- corrected as well.

-- ─────────────────────────────────────────────────────────────────
-- 1. get_dashboard_summary — agent_counts + campaign_agg
-- ─────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────
-- 3. get_campaign_pipeline_refresh
-- ─────────────────────────────────────────────────────────────────
