-- Migration 082: revert the lead_health feature entirely.
-- Removes the column, index, and refresh RPC introduced in migrations 077–079.
-- Restores add_lead_call_note, add_lead_plain_note, and update_lead_status
-- to their pre-health bodies (matching migrations 0030, 0040, and 0060).

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. Drop column and index
-- ─────────────────────────────────────────────────────────────────────────────

DROP INDEX IF EXISTS idx_leads_health;
ALTER TABLE leads DROP COLUMN IF EXISTS lead_health;

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. Drop refresh RPC
-- ─────────────────────────────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS refresh_lead_health_bulk();

-- ─────────────────────────────────────────────────────────────────────────────
-- 3. Restore add_lead_call_note (pre-health body from migration 0030)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 4. Restore add_lead_plain_note (pre-health body from migration 0040)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 5. Restore update_lead_status (pre-health body — matches migration 0060
--    which added resolution_reason persistence, the last pre-health version)
-- ─────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- 3. Update lead: status, status_changed_at, last_activity_at, resolution_reason
  UPDATE leads
     SET status            = p_status,
         ...
         resolution_reason = CASE
                               ...
                               WHEN p_status = 'in_discussion'   THEN NULL
