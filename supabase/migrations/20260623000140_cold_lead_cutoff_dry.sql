-- Migration 0140: DRY the going-cold cutoff — one SQL anchor, no hardcoded interval
--
-- WHY
-- ───
-- The going-cold window (COLD_LEAD_THRESHOLD_DAYS = 5, src/lib/constants/leads.ts)
-- was duplicated as a bare `interval '5 days'` literal inside get_dashboard_summary
-- and re-stamped by every CREATE OR REPLACE of that function (0081 → 0115 → 0129 →
-- 0138). The only thing keeping it in step with the TS constant was a code comment.
-- A change to the TS number would silently leave the dashboard widget on 5.
--
-- WHAT
-- ────
-- 1. public.cold_lead_cutoff() — THE single SQL source of the cold cutoff. Returns
--    `now() - interval '5 days'`. The `5` here and COLD_LEAD_THRESHOLD_DAYS in
--    src/lib/constants/leads.ts are the ONLY two places the number lives; change
--    them together. Rolling window (now − N days), matching every TS consumer.
-- 2. get_dashboard_summary — recreated from the LIVE body (the deployed function had
--    drifted ahead of the on-disk migrations: 0138-shape roster CTE +
--    pending_calls_count/new_leads_count). The ONLY change vs the live body is the
--    cold-leads predicate: `v_now - interval '5 days'` → `cold_lead_cutoff()`.
--    Everything else is byte-identical. This reconciles file ⇆ DB.
--
-- The TS side (getLeadsByRole / getLeadsForExport / getGoingColdLeads) calls the
-- new goingColdCutoff() helper; the count RPC (get_leads_status_counts) still
-- receives the cutoff as p_going_cold from getLeadsByRole, so it cannot drift.
--
-- NULL last_activity_at (never-contacted) is excluded by `<` (NULL < x is NULL) —
-- those leads are SLA-01A's job, not going-cold. Unchanged.
--
-- The GRANT line is non-negotiable: CREATE OR REPLACE silently drops the existing
-- GRANT; it must be re-applied immediately. (0102 revoke posture preserved — this
-- function is self/role-scoped and keeps its `authenticated` GRANT.)

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. The single cold-cutoff anchor
-- ─────────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.cold_lead_cutoff()
RETURNS timestamptz
LANGUAGE sql
STABLE
SET search_path = public
AS $$
  -- 5 == COLD_LEAD_THRESHOLD_DAYS in src/lib/constants/leads.ts (keep in sync).
  SELECT now() - interval '5 days';
$$;

COMMENT ON FUNCTION public.cold_lead_cutoff() IS
  'Going-cold cutoff: leads with last_activity_at before this are cold. The interval is pinned to COLD_LEAD_THRESHOLD_DAYS in src/lib/constants/leads.ts — change both together.';

-- ─────────────────────────────────────────────────────────────────────────────
-- 2. get_dashboard_summary — live body, cold predicate repointed to cold_lead_cutoff()
-- ─────────────────────────────────────────────────────────────────────────────
