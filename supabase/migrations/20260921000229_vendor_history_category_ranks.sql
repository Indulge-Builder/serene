-- 0229 — the category guess ranks the vendor history search; it no longer filters it.
--
-- WHY
-- 0228 stopped the search timing out. This is the second half of the same incident,
...
BEGIN
  ...
  hit AS (
    ...
    WHERE v.status = 'active'
      -- A removed vendor (0227 deleted_at) is never suggested. search_vendors and
      -- get_vendor_candidates already exclude it; this path did not, so a hidden
      -- vendor still surfaced in Find a vendor whenever an old title matched
      -- (found in review, 2026-09-21).
      AND v.deleted_at IS NULL
      -- The category and service are NOT filtered here any more (0229). They are a
  ...
  scored AS (
    ...
    -- The guess now RANKS instead of excluding (0229): a job under the category the
...
COMMENT ON FUNCTION public.find_vendors_by_history(text, text[], text, text, text, integer) IS
  ...
  'than 5% of titles is dropped before the join (0228). The category and service are a GUESS and RANK the result '
  '(+40% / +20% on agreement) rather than filtering it (0229): filtering on a chip that read "Special Request" hid the '
  'vendor whose job was filed under Retail, and the page answered a power bank request with airlines. A removed vendor (deleted_at) never appears. The city still '
