...
| `20260531000051_task_remark_rpc_auth_fix.sql` | `add_task_remark_with_status` — removes broken `auth.uid()` gate; `tasks_agent_select` adds `created_by = auth.uid()`; `task_remarks` SELECT/INSERT mirror tasks visibility (assignee, creator, manager/admin/founder); action gates on user-scoped tasks SELECT (view = post) |
